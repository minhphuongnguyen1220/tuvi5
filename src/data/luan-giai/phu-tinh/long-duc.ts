import type { DoanLuanGiai } from '../types';

/**
 * LUẬN GIẢI SAO LONG ĐỨC - Thủy, Thiện Tinh, vòng Thái Tuế (vị trí 8).
 * Tam hợp Âm-Long-Trực (Thiếu Âm + Long Đức + Trực Phù) - phần chung tam hợp đã viết tại thieu-am.ts.
 * Thuộc bộ TỨ ĐỨC (Long Đức + Phúc Đức + Thiên Đức + Nguyệt Đức) - chế Đào Hồng.
 * Là một trong các SAO GIẢI - giải tai nạn nhỏ, bệnh tật.
 */
export const luanGiai_LongDuc: DoanLuanGiai[] = [
  {
    id: 'long-duc-tinh-chat-chung',
    title: 'Long Đức - Đặc tính chung',
    sao: ['Long Đức'],
    doUuTien: 32,
    tomTat: `**Long Đức** — phụ tinh **THIỆN TINH** trong **vòng Thái Tuế**, vị trí thứ **8**.

**Ngũ hành:** **Thủy**.

**Hóa khí:** **Thiện Tinh**.

**Vị thế trong vòng Thái Tuế:**
- Tam hợp **ÂM - LONG - TRỰC** (Thiếu Âm + Long Đức + Trực Phù) — tam hợp của
  sự **KHÔN NGOAN, NHÚN NHƯỜNG, CHỊU THIỆT THÒI**

**Vai trò:**
- Đại diện về **NHU THUẬN, NHÂN HẬU, ĐOAN CHÍNH, CAO THƯỢNG**
- **GIẢI TRỪ NẠN NHỎ, ỐM ĐAU**
- Cùng với **Tứ Đức**, có thể **CHẾ ĐƯỢC một phần TÍNH HOA NGUYỆT, DÂM ĐÃNG**
  của Đào Hồng: biến thành người **CÓ DUYÊN, PHẨM HẠNH**, thu hút bằng tính
  nết **đoan trang, không lả lơi trêu cợt**

**Đặc tính:** **KHOAN HÒA, NHÂN HẬU, ĐOAN CHÍNH, PHÚC THIỆN, GIẢI ĐƯỢC TAI NẠN
HUNG NGUY**.`,
  },

  {
    id: 'long-duc-tu-duc-che-dao-hong',
    title: 'Bộ Tứ Đức (Long Đức + Phúc Đức + Thiên Đức + Nguyệt Đức) - chế Đào Hồng',
    sao: ['Long Đức', 'Phúc Đức', 'Thiên Đức', 'Nguyệt Đức'],
    doUuTien: 70,
    tomTat: `**Bộ TỨ ĐỨC** = **Long Đức + Phúc Đức + Thiên Đức + Nguyệt Đức** —
4 sao **ĐỨC** quan trọng trong tử vi.

**Tác dụng đặc biệt:** có thể **CHẾ ĐƯỢC** một phần tính **HOA NGUYỆT, DÂM
ĐÃNG** của bộ **ĐÀO HỒNG** (Đào Hoa + Hồng Loan):
- Biến đào hoa từ **lả lơi, dâm tà** thành **CÓ DUYÊN, PHẨM HẠNH**
- Người **THU HÚT** người khác bằng tính nết **ĐOAN TRANG**
- Không **lả lơi trêu cợt**

**Cách cục đẹp:**
- Người có **đào hoa** vốn dễ sa ngã: khi có Tứ Đức: trở thành **MỸ NHÂN ĐOAN
  CHÍNH**, được **TÔN TRỌNG**
- Phụ nữ có **Đào Hồng + Tứ Đức** đặc biệt **ĐẸP NHÂN HẬU**, **PHÚC HẬU**

- Đây là một trong những **CÁCH HÓA GIẢI** đào hoa hiệu quả nhất — cùng với
**Vũ Khúc** và **Thiên Hình** đã đề cập.`,
  },

  {
    id: 'long-duc-sao-giai',
    title: 'Long Đức là Sao Giải - tăng hiệu lực giải tinh',
    sao: ['Long Đức'],
    doUuTien: 50,
    tomTat: `**Long Đức là một trong các SAO GIẢI quan trọng:**

Nhờ **NHÂN HẬU, ÔN HÒA**, Long Đức có khả năng **GIẢI TRỪ**:
- **Bệnh tật và tai họa NHỎ**
- Mang lại **MAY MẮN, HỶ SỰ**

**Các sao GIẢI tinh tương tự:**
- **Thiên Giải, Địa Giải, Giải Thần** (3 sao Giải chuyên biệt)
- **Thiên Quan, Thiên Phúc** (cứu giải)
- **Thiếu Âm, Thiếu Dương** (sao của vòng Thái Tuế)
- **Quang Quý** (Ân Quang + Thiên Quý)
- **Tả Phù, Hữu Bật** (Lục Cát Tả Hữu)
- **Tràng Sinh, Đế Vượng** (vòng Tràng Sinh)

**CÀNG NHIỀU GIẢI TINH: CÀNG TĂNG HIỆU LỰC CỨU GIẢI**.

- Cách cục đẹp: lá số có **NHIỀU GIẢI TINH** đồng cung/hợp chiếu: đương số
**dễ thoát hiểm** trong tình huống **THẬP TỬ NHẤT SINH**.`,
  },

  {
    id: 'long-duc-tai-menh',
    title: 'Long Đức tại Mệnh - nhân hậu từ bi',
    sao: ['Long Đức'],
    cung: ['Mệnh'],
    doUuTien: 78,
    tomTat: `**Long Đức thủ Mệnh:**

**Tính cách nổi bật:**
- **ÔN HÒA, NHÂN HẬU, ĐỨC HẠNH, ĐẸP NẾT**
- **HIỀN LÀNH, KHOAN HÒA**, sống **NHÂN NGHĨA**
- **TỪ BI, từ thiện**, hay **GIÚP ĐỠ, LÀM PHÚC**
- Biết **NGHĨ CHO NGƯỜI KHÁC**

**Phúc thọ:**
Vì "**Ở HIỀN GẶP LÀNH**" nên chủ mệnh thường:
- Gặp **QUÝ NHÂN PHÙ TRỢ**
- Đi đâu cũng có **NGƯỜI THƯƠNG MẾN**

**Khả năng GIẢI HỌA:**
- **GIẢM NHẸ BỆNH TẬT**
- **HÓA GIẢI TAI ƯƠNG**
- Vượt qua **những tình huống "thập tử nhất sinh"**
- Tâm tính và niềm tin **xuyên suốt** giúp đương số đứng vững`,
  },

  {
    id: 'long-duc-co-nguyet-dong-luong-quang-quy',
    title: 'Long Đức + Cơ Nguyệt Đồng Lương + Quang Quý + Quan Phúc - căn tu',
    sao: ['Long Đức'],
    ketHop: ['Thiên Cơ', 'Thái Âm', 'Thiên Đồng', 'Thiên Lương', 'Ân Quang', 'Thiên Quý', 'Thiên Quan', 'Thiên Phúc'],
    doUuTien: 75,
    tomTat: `**Long Đức + Cơ Nguyệt Đồng Lương + Ân Quang + Thiên Quý + Thiên Quan + Thiên Phúc:**

- Người có **THIỆN TÂM**, **CĂN TU**:
- Dù không phải **TU SỸ** thì cũng là **CƯ SỸ, TỲ KHEO**
- Tấm lòng **NHÂN ÁI** bộc lộ rõ
- Sẵn sàng **DANG TAY GIÚP ĐỠ** người khác
- **Không tính toán thiệt hơn**
- "**CHO ĐI CHẲNG CẦN NHẬN LẠI**"

**Tâm sáng**: dễ **BÉN DUYÊN VỚI CON ĐƯỜNG TU ĐẠO** hoặc sống một đời có
**CHUẨN MỰC, ĐẠO LÝ**.

- Đây là cách **TÂM LINH ĐẸP NHẤT** của Long Đức — nhân nghĩa kết hợp với cách
cục Cơ Nguyệt Đồng Lương + quý tinh tâm linh.`,
  },

  {
    id: 'long-duc-tai-phu-mau',
    title: 'Long Đức tại Phụ Mẫu',
    sao: ['Long Đức'],
    cung: ['Phụ Mẫu'],
    doUuTien: 75,
    tomTat: `**Long Đức tại Phụ Mẫu:**
- Cha mẹ là người **NHÂN HẬU, TỪ THIỆN**, có **NGHỀ KHÉO**
- Cha mẹ có **ĐỨC TIN**, **theo đạo** hoặc **sùng bái tôn giáo** — đặc biệt rõ
  khi về già
- Con cái và cha mẹ **HÒA THUẬN** — chung niềm tin gia đình: mối quan hệ
  **sâu sắc**

**Quy luật ĐỐI XUNG:**
Long Đức thủ Phụ Mẫu: đối cung luôn có **TUẾ PHÁ** chiếu về.
- Người có cha mẹ **HIỀN LÀNH, NHẸ NHÀNG** nhưng bản thân thường **BỐC ĐỒNG,
  HAY CHỐNG ĐỐI**, đi ngược lại
- Đây là cách rất hay gặp: con của người hiền hậu lại **ngỗ nghịch**`,
  },

  {
    id: 'long-duc-phu-mau-xuong-khuc',
    title: 'Long Đức Phụ Mẫu + Văn Xương / Văn Khúc - đồng cảm thấu hiểu',
    sao: ['Long Đức'],
    cung: ['Phụ Mẫu'],
    ketHop: ['Văn Xương', 'Văn Khúc'],
    doUuTien: 70,
    tomTat: `**Long Đức Phụ Mẫu + Văn Xương / Văn Khúc:** cha mẹ và con cái dễ **ĐỒNG CẢM, YÊU THƯƠNG, THẤU HIỂU** nhau.`,
  },

  {
    id: 'long-duc-phu-mau-sat-hinh-ky',
    title: 'Long Đức Phụ Mẫu + Thất Sát + Thiên Hình + Hóa Kỵ - mâu thuẫn rồi hóa giải',
    sao: ['Long Đức'],
    cung: ['Phụ Mẫu'],
    ketHop: ['Thất Sát', 'Thiên Hình', 'Hóa Kỵ'],
    doUuTien: 70,
    tomTat: `**Long Đức Phụ Mẫu + Thất Sát + Thiên Hình + Hóa Kỵ:** ban đầu có khả năng **MÂU THUẪN**, nhưng chính mâu thuẫn ấy lại là **CHẤT XÚC TÁC** giúp hai bên **THAY ĐỔI NHẬN THỨC**, từ đó **HÓA GIẢI BẤT ĐỒNG**.`,
  },

  {
    id: 'long-duc-tai-phuc-duc',
    title: 'Long Đức tại Phúc Đức',
    sao: ['Long Đức'],
    cung: ['Phúc Đức'],
    doUuTien: 78,
    tomTat: `**Long Đức tại Phúc Đức là ĐIỀM LÀNH:**
- Tổ tiên vốn là người **LƯƠNG THIỆN**, coi trọng **chữ "ĐỨC"**
- Họ hàng **HIỀN LÀNH, PHÚC ĐỨC**, **NHƯỜNG NHỊN GIÚP ĐỠ NHAU**
- Người trong dòng họ có **NIỀM TIN TÔN GIÁO** mạnh mẽ, hoặc **niềm tin vào
  TỔ TIÊN**

**Đặc điểm tâm linh:**
- **MỒ MẢ THUẬN VỊ** — phù hộ con cháu **TỐT LÀNH**
- Có **CON SÔNG, DÒNG SUỐI NHỎ** ở gần
- Đương số được **THỪA HƯỞNG PHÚC ẤM tổ tiên**
- Có **TINH THẦN KIÊN ĐỊNH**, được **NHIỀU NGƯỜI TÍN NHIỆM**`,
  },

  {
    id: 'long-duc-phuc-duc-khong-kiep',
    title: 'Long Đức Phúc Đức + Địa Không / Địa Kiếp - lánh đời đi tu',
    sao: ['Long Đức'],
    cung: ['Phúc Đức'],
    ketHop: ['Địa Không', 'Địa Kiếp'],
    doUuTien: 73,
    tomTat: `**Long Đức Phúc Đức + Địa Không / Địa Kiếp:** sinh tâm lý **BUÔNG BỎ**, muốn **"LÁNH ĐỜI ĐI TU"**.`,
  },

  {
    id: 'long-duc-phuc-duc-hung-tinh',
    title: 'Long Đức Phúc Đức + Hung Tinh - niềm tin lung lay',
    sao: ['Long Đức'],
    cung: ['Phúc Đức'],
    ketHop: ['Kình Dương', 'Đà La', 'Hỏa Tinh', 'Linh Tinh', 'Địa Không', 'Địa Kiếp'],
    doUuTien: 73,
    tomTat: `**Long Đức Phúc Đức + Hung Tinh:**
- Niềm tin **DỄ LUNG LAY**
- Ảnh hưởng **sự VỮNG BỀN của GIA NGHIỆP**`,
  },

  {
    id: 'long-duc-tai-dien-trach',
    title: 'Long Đức tại Điền Trạch',
    sao: ['Long Đức'],
    cung: ['Điền Trạch'],
    doUuTien: 75,
    tomTat: `**Long Đức tại Điền Trạch:**
- Đi đâu cũng được **GIÚP ĐỠ về NHÀ CỬA**, nơi ăn chốn ở
- Quan hệ **HÀNG XÓM TỐT ĐẸP**, **THÂN QUÝ NHAU**
- **Sẵn sàng giúp đỡ nhau**

**Đặc điểm nhà cửa:**
- Nhà ở **SẠCH SẼ, GỌN GÀNG, GIẢN DỊ**
- **THỜ CÚNG TỐT**`,
  },

  {
    id: 'long-duc-dien-trach-tu-phu-dong-luong-quang-quy',
    title: 'Long Đức Điền + Tử Phủ Đồng Lương + Quang Quý - rước Phật về thờ',
    sao: ['Long Đức'],
    cung: ['Điền Trạch'],
    ketHop: ['Tử Vi', 'Thiên Phủ', 'Thiên Đồng', 'Thiên Lương', 'Ân Quang', 'Thiên Quý'],
    doUuTien: 70,
    tomTat: `**Long Đức Điền Trạch + Tử Phủ Đồng Lương + Ân Quang + Thiên Quý:** nhà dễ **RƯỚC PHẬT VỀ THỜ** — gia đình trở thành **đạo tràng** nhỏ.`,
  },

  {
    id: 'long-duc-dien-trach-tu-duc-tu-linh',
    title: 'Long Đức Điền + Tứ Đức / Tứ Linh - đất thiêng',
    sao: ['Long Đức'],
    cung: ['Điền Trạch'],
    ketHop: ['Long Đức', 'Phúc Đức', 'Thiên Đức', 'Nguyệt Đức', 'Long Trì', 'Phượng Các', 'Bạch Hổ', 'Hoa Cái'],
    doUuTien: 70,
    tomTat: `**Long Đức Điền Trạch + Tứ Đức / Tứ Linh:** điềm **ĐẤT THIÊNG** — gia chủ nên **GIỮ GÌN VIỆC THỜ PHỤNG** vì "**có thờ có thiêng, có kiêng có lành**".`,
  },

  {
    id: 'long-duc-dien-trach-rieu-da-ky-linh',
    title: 'Long Đức Điền + Riêu / Đà / Kỵ / Linh - phong thủy không sạch',
    sao: ['Long Đức'],
    cung: ['Điền Trạch'],
    ketHop: ['Thiên Riêu', 'Đà La', 'Hóa Kỵ', 'Linh Tinh'],
    doUuTien: 70,
    tomTat: `**Long Đức Điền Trạch + Thiên Riêu / Đà La / Hóa Kỵ / Linh Tinh:** phong thủy nơi ở dễ có **CHỖ KHÔNG SẠCH SẼ** — cần kiểm tra kỹ.`,
  },

  {
    id: 'long-duc-dien-trach-kiep-kinh-ky',
    title: 'Long Đức Điền + Địa Kiếp / Kình Dương / Hóa Kỵ - lời ra tiếng vào',
    sao: ['Long Đức'],
    cung: ['Điền Trạch'],
    ketHop: ['Địa Kiếp', 'Kình Dương', 'Hóa Kỵ'],
    doUuTien: 70,
    tomTat: `**Long Đức Điền Trạch + Địa Kiếp / Kình Dương / Hóa Kỵ:** dễ sinh chuyện **"LỜI RA TIẾNG VÀO"**, **BẤT ĐỒNG về niềm tin** hay chuyện **THỜ CÚNG**.`,
  },

  {
    id: 'long-duc-tai-quan-loc',
    title: 'Long Đức tại Quan Lộc',
    sao: ['Long Đức'],
    cung: ['Quan Lộc'],
    doUuTien: 78,
    tomTat: `**Long Đức tại Quan Lộc:**
- Trong công việc luôn **ĐIỀM ĐẠM, NHẸ NHÀNG**
- **HAY GẶP MAY**, có **LƯƠNG TÂM nghề nghiệp**
- **TRÊN BIẾT KÍNH, DƯỚI BIẾT NHƯỜNG**, vô cùng điềm đạm
- **NHIỀU NGƯỜI QUÝ MẾN**
- Làm việc bằng **CÁI TÂM**, **TẬN TỤY CỐNG HIẾN**`,
  },

  {
    id: 'long-duc-quan-loc-co-nguyet-dong-luong-quang-quy',
    title: 'Long Đức Quan + Cơ Nguyệt Đồng Lương + Quang Quý - nghề cứu giúp người',
    sao: ['Long Đức'],
    cung: ['Quan Lộc'],
    ketHop: ['Thiên Cơ', 'Thái Âm', 'Thiên Đồng', 'Thiên Lương', 'Ân Quang', 'Thiên Quý'],
    doUuTien: 73,
    tomTat: `**Long Đức Quan Lộc + Cơ Nguyệt Đồng Lương + Ân Quang + Thiên Quý:** người hay làm nghề **CỨU GIÚP NGƯỜI**:
- **THẦY THUỐC, BÁC SỸ**
- **GIÁO VIÊN**
- Tổ chức **XÃ HỘI, TỪ THIỆN**
- **BẢO VỆ ĐỘNG VẬT**`,
  },

  {
    id: 'long-duc-quan-loc-thien-luong',
    title: 'Long Đức Quan + Thiên Lương - hổ mọc thêm cánh',
    sao: ['Long Đức'],
    cung: ['Quan Lộc'],
    ketHop: ['Thiên Lương'],
    doUuTien: 73,
    tomTat: `**Long Đức Quan Lộc + Thiên Lương:** như "**HỔ MỌC THÊM CÁNH**" — càng dễ gặp **QUÝ NHÂN PHÙ TRỢ** trong công việc.`,
  },

  {
    id: 'long-duc-quan-loc-hoa-ky',
    title: 'Long Đức Quan + Hóa Kỵ - công danh chậm chạp',
    sao: ['Long Đức'],
    cung: ['Quan Lộc'],
    ketHop: ['Hóa Kỵ'],
    doUuTien: 73,
    tomTat: `**Long Đức Quan Lộc + Hóa Kỵ:**
- Con đường công danh **CHẬM CHẠP**, dễ **CHỊU THIỆT THÒI**
- Dù bản thân **không than phiền**, vẫn **hết lòng làm việc**
- Phải **CHỜ THỜI GIAN DÀI** mới có được thành quả`,
  },

  {
    id: 'long-duc-tai-no-boc',
    title: 'Long Đức tại Nô Bộc',
    sao: ['Long Đức'],
    cung: ['Nô Bộc'],
    doUuTien: 72,
    tomTat: `**Long Đức tại Nô Bộc:**
- **NHIỀU BẠN BÈ, ĐỒNG NGHIỆP, CẤP DƯỚI** TỐT, **ĐÔN HẬU, TỪ THIỆN**
- Bạn bè đa số **TÂM TÍNH THIỆN LÀNH**
- Biết **LẮNG NGHE**, luôn cố gắng truyền **NĂNG LƯỢNG TÍCH CỰC**
- Khi gặp vấn đề, hay được bạn bè **GIÚP ĐỠ**, ngồi **NGHE TÂM SỰ**
- **KHÔNG BAO GIỜ BỎ MẶC**`,
  },

  {
    id: 'long-duc-no-boc-am-tinh',
    title: 'Long Đức Nô + ám tinh - bạn bè niềm tin lệch lạc',
    sao: ['Long Đức'],
    cung: ['Nô Bộc'],
    ketHop: ['Thiên Riêu', 'Thiên Y', 'Đà La', 'Linh Tinh', 'Địa Kiếp'],
    doUuTien: 67,
    tomTat: `**Long Đức Nô Bộc + Thiên Riêu / Thiên Y / Đà La / Linh Tinh / Địa Kiếp** (ám tinh):
- Bạn bè dễ có **NIỀM TIN LỆCH LẠC**
- Có thể **KÉO chủ mệnh vào tổ chức tôn giáo KHÔNG PHÙ HỢP**
- Cần **CẢNH GIÁC** trước những lời mời gọi tâm linh khả nghi`,
  },

  {
    id: 'long-duc-tai-thien-di',
    title: 'Long Đức tại Thiên Di',
    sao: ['Long Đức'],
    cung: ['Thiên Di'],
    doUuTien: 72,
    tomTat: `**Long Đức tại Thiên Di:**
- Ra ngoài **KHIÊM TỐN**, được **MỌI NGƯỜI NỂ TRỌNG**
- Hay gặp **QUÝ NHÂN**
- Dáng vẻ **ĐIỀM ĐẠM, MẪU MỰC, TỪ TỐN**
- Thường hay **NHƯỜNG NHỊN, NHẸ NHÀNG**, hay **GIÚP ĐỠ MỌI NGƯỜI**

**Khả năng cứu giải:** đôi khi vì **LẠC QUAN QUÁ MỨC** mà **VA PHẢI NGHỊCH
CẢNH** — lúc đó lại may mắn **GẶP QUÝ NHÂN CỨU GIÚP**.`,
  },

  {
    id: 'long-duc-thien-di-pha-sat-kinh-hoa',
    title: 'Long Đức Thiên Di + Phá / Sát / Kình / Hỏa - giảm vận may',
    sao: ['Long Đức'],
    cung: ['Thiên Di'],
    ketHop: ['Phá Quân', 'Thất Sát', 'Kình Dương', 'Hỏa Tinh'],
    doUuTien: 67,
    tomTat: `**Long Đức Thiên Di + Phá Quân / Thất Sát / Kình Dương / Hỏa Tinh** (sao "ĐỘNG" mạnh, quyết liệt):
- **GIẢM BỚT VẬN MAY**
- **KHÓ NHẬN ĐƯỢC sự trợ giúp KỊP THỜI**
- Vì các sao động phá vỡ tính tĩnh tại của Long Đức.`,
  },

  {
    id: 'long-duc-tai-tat-ach',
    title: 'Long Đức tại Tật Ách',
    sao: ['Long Đức'],
    cung: ['Tật Ách'],
    doUuTien: 78,
    tomTat: `**Long Đức tại Tật Ách:**
- Bằng cách **SỐNG ĐÔN HẬU, KHIÊM NHƯỜNG**, có khả năng **GIẢI TRỪ**:
  - **TAI NẠN, BỆNH TẬT NHỎ**
- Chỉ **ỐM VẶT**, ít khi mắc bệnh nặng

**Bệnh có thể gặp** (do Long Đức thuộc **HÀNH THỦY**):
- **MÁU HUYẾT**
- **ĐƯỜNG RUỘT**
- **ĐƯỜNG TIÊU HÓA**`,
  },

  {
    id: 'long-duc-tat-ach-sat-hinh-ky',
    title: 'Long Đức Tật + Sát - Hình - Kỵ - bệnh tật thử thách',
    sao: ['Long Đức'],
    cung: ['Tật Ách'],
    ketHop: ['Kình Dương', 'Đà La', 'Hỏa Tinh', 'Linh Tinh', 'Địa Không', 'Địa Kiếp', 'Thiên Hình', 'Hóa Kỵ'],
    doUuTien: 73,
    tomTat: `**Long Đức Tật Ách + Sát - Hình - Kỵ:** bệnh đến như một **THỬ THÁCH**:
- Buộc đương số phải **VỮNG LÒNG TIN**
- Phải **CHỊU THIỆT THÒI**, bị bệnh tật **HÀNH HẠ** mới thúc đẩy **QUÝ NHÂN** đến hóa giải.`,
  },

  {
    id: 'long-duc-tai-tai-bach',
    title: 'Long Đức tại Tài Bạch',
    sao: ['Long Đức'],
    cung: ['Tài Bạch'],
    doUuTien: 75,
    tomTat: `**Long Đức tại Tài Bạch:**
- Tiền bạc đến theo kiểu **"CHẬM MÀ CHẮC"**
- **KIẾM TIỀN CHÍNH ĐÁNG, TRONG SẠCH**
- Có **LÒNG TỪ THIỆN cao**, **ÍT MUỐN TRANH GIÀNH** với ai
- Hay **GẶP MAY** về tiền
- Sẵn sàng **GIÚP ĐỠ, QUYÊN GÓP TỪ THIỆN, LÀM PHÚC**

**Đặc tính tài chính:**
- **KHÔNG PHÙ HỢP** với lối **làm giàu BẠO PHÁT**
- Đôi khi phải **CHỊU THIỆT THÒI** tiền bạc — nhưng là cách **"LÙI MỘT BƯỚC,
  TIẾN BA BƯỚC"**

**Mặt tích cực:**
Dù **GIẢM BỚT** sự **bùng nổ** trong tài chính, nhưng đổi lại chủ mệnh sẽ:
- **TRÁNH ĐƯỢC RỦI RO**
- Cuộc sống **ỔN ĐỊNH** lâu dài`,
  },

  {
    id: 'long-duc-tai-tu-tuc',
    title: 'Long Đức tại Tử Tức',
    sao: ['Long Đức'],
    cung: ['Tử Tức'],
    doUuTien: 72,
    tomTat: `**Long Đức tại Tử Tức:**
- Con cái **HIỀN LÀNH, DỄ CHỊU**, **HÒA THUẬN, HIẾU THẢO, NGOAN NGOÃN**
- **NHƯỜNG NHỊN NHAU**

**Đặc tính:**
- Dù cha mẹ đôi lúc **NÓNG NẢY, KHẮT KHE** thì con vẫn giữ thái độ **HÒA THUẬN**
- "**Một điều nhịn, chín điều lành**" — không phản kháng hay chống đối
- Mối quan hệ trong gia đình **HÀI HÒA**, **ÍT XẢY RA XUNG ĐỘT**
- Tình cảm **không bộc lộ rõ** bằng lời nói nhưng vẫn có sự **GẮN BÓ ÂM THẦM**`,
  },

  {
    id: 'long-duc-tu-tuc-ky-cu-mon-da',
    title: 'Long Đức Tử Tức + Kỵ / Cự Môn / Đà La - bằng mặt không bằng lòng',
    sao: ['Long Đức'],
    cung: ['Tử Tức'],
    ketHop: ['Hóa Kỵ', 'Cự Môn', 'Đà La'],
    doUuTien: 67,
    tomTat: `**Long Đức Tử Tức + Hóa Kỵ / Cự Môn / Đà La** (ám tinh):
- Sự hiền hòa của con cái chuyển thành **THÁI ĐỘ CAM CHỊU**
- "**BẰNG MẶT MÀ KHÔNG BẰNG LÒNG**"
- Sự **TIN TƯỞNG GIẢM**, mối quan hệ **XA CÁCH**`,
  },

  {
    id: 'long-duc-tai-phu-the',
    title: 'Long Đức tại Phu Thê - tướng phu thê',
    sao: ['Long Đức'],
    cung: ['Phu Thê'],
    doUuTien: 75,
    tomTat: `**Long Đức tại Phu Thê:**
- Vợ chồng **BIẾT NHƯỜNG NHỊN**, **GIỮ GÌN HẠNH PHÚC**
- Người hôn phối là người **HIỀN HẬU, NHẸ NHÀNG, ĐỨC ĐỘ**
- Sống **TÌNH NGHĨA**, **NHƯỜNG NHỊN**, **LÂU BỀN**
- **ÍT KHI XẢY RA MÂU THUẪN**

**Tướng phu thê:**
- Hai vợ chồng **TƯỚNG ĐỐI XỨNG ĐÔI VỪA LỨA**
- Hay thích **LÀM PHƯỚC THIỆN, TU TẬP** cùng nhau
- Ở giai đoạn mới yêu, dù chỉ ở cạnh nhau **THỜI GIAN NGẮN** cũng đã xuất hiện
  **TƯỚNG PHU THÊ**`,
  },

  {
    id: 'long-duc-phu-the-dia-khong',
    title: 'Long Đức Phu Thê + Địa Không - chịu đựng tổn thương',
    sao: ['Long Đức'],
    cung: ['Phu Thê'],
    ketHop: ['Địa Không'],
    doUuTien: 70,
    tomTat: `**Long Đức Phu Thê + Địa Không:**
- Long Đức **KHÔNG hóa giải hết** sát khí
- Bên ngoài **CHỊU ĐỰNG, NHẪN NHỊN** xuất hiện
- Trong lòng **chất chứa TỔN THƯƠNG khó nguôi ngoai**`,
  },

  {
    id: 'long-duc-tai-huynh-de',
    title: 'Long Đức tại Huynh Đệ',
    sao: ['Long Đức'],
    cung: ['Huynh Đệ'],
    doUuTien: 70,
    tomTat: `**Long Đức tại Huynh Đệ:**
- Anh chị em **THƯƠNG YÊU NHAU**, **BIẾT NHƯỜNG NHỊN NHAU**
- Coi trọng **TÌNH THÂN**
- Sống **HÒA THUẬN, HIỀN HẬU**, có **TÌNH NGHĨA**
- Sẵn sàng **GIÚP ĐỠ** nhau khi cần
- Dù xa cách nhưng vẫn **NHỚ VỀ NHAU**`,
  },

  {
    id: 'long-duc-huynh-de-pha-hao-khong-kiep',
    title: 'Long Đức Huynh Đệ + Phá / Hao / Không / Kiếp - giúp đỡ thành gánh nặng',
    sao: ['Long Đức'],
    cung: ['Huynh Đệ'],
    ketHop: ['Phá Quân', 'Đại Hao', 'Tiểu Hao', 'Địa Không', 'Địa Kiếp'],
    doUuTien: 65,
    tomTat: `**Long Đức Huynh Đệ + Phá Quân / Đại Hao / Tiểu Hao / Không Kiếp:**
- Việc giúp đỡ **KHÔNG TẠO ĐƯỢC kết quả tốt**
- **TÂM MUỐN GIÚP nhưng VÔ TÌNH GÂY RẮC RỐI, CẢN TRỞ**
- Lòng tốt thành **gánh nặng** cho người được giúp`,
  },

  {
    id: 'long-duc-vao-han',
    title: 'Long Đức vào hạn',
    sao: ['Long Đức'],
    doUuTien: 50,
    tomTat: `**Vận có Long Đức:** giai đoạn con người trở nên:
- **ĐIỀM ĐẠM, NHẸ NHÀNG**
- **KHÔNG HAM QUYỀN ĐOẠT VỊ**
- Hay **GIÚP ĐỠ NGƯỜI KHÁC** và được **NGƯỜI KHÁC GIÚP ĐỠ LẠI**
- Ứng với câu "**Ở hiền thì gặp lành**"

**Đặc điểm vận hạn này:**
- **GIẢI TRỪ** được nhiều **TAI HỌA NHỎ**
- Giúp đương số **vượt qua khó khăn** một cách **bình an**
- **Tâm bình an**, vận trình **THUẬN LỢI** dù không bùng nổ

- Đây là **HẠN BÌNH AN**, đặc biệt tốt cho người vừa trải qua hạn dữ dội —
giúp **HỒI PHỤC** sau biến cố.`,
  },

  {
    id: 'long-duc-loi-khuyen',
    title: 'Lời khuyên cho người Long Đức',
    sao: ['Long Đức'],
    doUuTien: 35,
    tomTat: `**Long Đức tượng trưng cho THIỆN TÂM, LÒNG TỪ BI và PHÚC ĐỨC.**

**Bùa hộ mệnh tự nhiên:**
- Dù đương số gặp nạn cũng **DỄ CÓ QUÝ NHÂN TRỢ GIÚP**
- Là sao **GIẢI** mạnh, hóa nguy thành an

**Bí quyết phát huy Long Đức:**
- **GIỮ CHỮ "NHÂN"**, **CHỮ "ĐỨC"**
- **TÍCH CỰC LÀM PHÚC, làm THIỆN**
- **THỜ CÚNG TỔ TIÊN** chu đáo
- **TU TÂM, TU KHẨU**
- Tránh các **ám tinh** kéo lệch tâm trí

- Khi giữ được **chữ "Nhân", chữ "Đức"** thì Long Đức chẳng khác nào **BÙA HỘ
MỆNH**, khiến cuộc đời:
- **HANH THÔNG**
- **BÌNH AN**
- **Vạn sự thuận lòng**`,
  },
];
