import type { DoanLuanGiai } from '../types';

/**
 * LUẬN GIẢI SAO QUAN PHỦ - Hỏa, Phù Tinh + Hình Tinh, vòng Lộc Tồn (vị trí 12 - CUỐI)
 * LUÔN xung chiếu với Tấu Thư - chủ về lý luận, pháp luật, kiện tụng
 */
export const luanGiai_QuanPhu: DoanLuanGiai[] = [
  {
    id: 'quan-phu-tinh-chat-chung',
    title: 'Quan Phủ - Đặc tính chung',
    sao: ['Quan Phủ'],
    doUuTien: 30,
    tomTat: `**Quan Phủ** — phụ tinh **PHÙ TINH + HÌNH TINH** trong **vòng Lộc Tồn**
(vị trí thứ **12 - cuối cùng**).

**Ngũ hành:** **Hỏa**.

**Hóa khí:** **Phù Tinh, Hình Tinh**.

⚡ **Vị thế trong vòng:**
- Nằm trong **TAM HỢP với Hỷ Thần và Tiểu Hao**
- **LUÔN xung chiếu với Tấu Thư** (cách 6 cung)

**Đặc tính:** **lý luận, quan tụng, thông minh, sắc bén, KHẮT KHE**.

⚡ **Bản chất 2 mặt:**
- 🌟 **Mặt tốt:** lý luận sắc bén, soi xét đúng sai, **MINH BẠCH** trong luật
  pháp → người **cầm cân nảy mực**
- ⚠️ **Mặt xấu:** **KIỆN CÁO, THỊ PHI**, biện bác, đơn từ, **tranh chấp**

⚡ Vì xung chiếu Tấu Thư (đơn từ) → **chủ về ĐƠN KIỆN, THƯA KIỆN, biện luận**.`,
  },

  {
    id: 'quan-phu-tai-menh',
    title: 'Quan Phủ tại Mệnh',
    sao: ['Quan Phủ'],
    cung: ['Mệnh'],
    doUuTien: 78,
    tomTat: `🌟 **Quan Phủ thủ Mệnh:** người có **KHẢ NĂNG XÉT ĐOÁN, LÝ LUẬN TỐT**,
đôi khi **hơi KHẮT KHE**, tính toán nhiều.

🌟 **+ Văn Xương / Văn Khúc / Thái Tuế:** lợi cho **PHÁP LUẬT** — kiện cáo, thị
phi, **biện hộ, bút chiến**. Cần thiết cho:
- **Thẩm phán, luật sư**
- **Thanh tra, kiểm toán**
- **Điều tra, giám sát**

→ Mẫu người làm việc theo **NGUYÊN TẮC và LOGIC**, "**người cầm cân nảy mực**".

⚠️ **+ Sát bại tinh:** người **HÈN HẠ, gian nịnh, phản bội**:
- **Cố oán, ưa trả thù**, thích kiện tụng, **câu chấp, đố kỵ**
- HOẶC bị **oán, báo thù, kiện, phản bội**

⚠️ **Tinh thần CẠNH TRANH, ĐỐ KỴ, NẶNG BÁO PHỤC** — sẵn sàng dùng thủ đoạn
tranh thắng → khơi mào **trả đũa**.

⚡ **Quan Phủ tại Mệnh / Thân:** báo hiệu **NGHIỆP CHƯỚNG** của việc **báo
oán, thù dai, thị phi**.`,
  },

  {
    id: 'quan-phu-tai-phu-mau',
    title: 'Quan Phủ tại Phụ Mẫu',
    sao: ['Quan Phủ'],
    cung: ['Phụ Mẫu'],
    doUuTien: 70,
    tomTat: `🌟 **Quan Phủ cư Phụ Mẫu + cát tinh + văn tinh:** cha mẹ làm nghề
**hành pháp, pháp luật**, hành chính, các nghề đòi hỏi **công tâm, minh
bạch**.

⚠️ Cha mẹ dễ **BẤT HÒA, cãi nhau** vì trái ngược quan điểm.

⚠️ **+ Sát bại tinh:** không chỉ mâu thuẫn thông thường mà dễ dẫn đến:
- **Chia tay, ly dị**
- **Kiện tụng nhau**
- Xa cách vì **bất hòa tư tưởng**`,
  },

  {
    id: 'quan-phu-tai-phuc-duc',
    title: 'Quan Phủ tại Phúc Đức',
    sao: ['Quan Phủ'],
    cung: ['Phúc Đức'],
    doUuTien: 70,
    tomTat: `⚠️ **Quan Phủ cư Phúc Đức:** trong dòng họ nhiều người **ƯA LÝ LẼ**,
dễ sa vào **TRANH CÃI, KIỆN TỤNG** lẫn nhau vì bất đồng quan điểm.

⚡ Mâu thuẫn thường về **quyền lợi, đất đai, hương hỏa, mồ mả tổ tiên**.

🌟 **+ Cát tinh + Quyền tinh:** ông bà tổ tiên có người **LÀM QUAN LỚN**, khắt
khe và **quyền biến**.

⚠️ Dễ có **bất đồng, cãi nhau** trong việc **THỜ CÚNG, mồ mả tổ tiên**.

⚡ **Đặc trưng hôn phối:** gia đình người hôn phối **KHÓ TÍNH, khắt khe**, hay
soi xét hoặc có người làm về **NGÀNH LUẬT**.`,
  },

  {
    id: 'quan-phu-tai-dien-trach',
    title: 'Quan Phủ tại Điền Trạch',
    sao: ['Quan Phủ'],
    cung: ['Điền Trạch'],
    doUuTien: 72,
    tomTat: `⚡ **Quan Phủ cư Điền:** nhà cửa **GẦN nơi CÔNG MÔN, CÔNG QUYỀN** —
**TÒA ÁN, VIỆN KIỂM SÁT, CÔNG AN, ỦY BAN**.

⚠️ Người **dễ mắc TRANH TỤNG, KIỆN CÁO** về **điền sản, đất đai**:
- Rắc rối **giấy tờ, quyền sử dụng đất**
- Tranh chấp **chia tài sản**

⚠️ Người **khó tính** hoặc **hàng xóm khó tính** → dễ **bất đồng**, dễ bị
**thưa kiện** vì những điều nhỏ nhặt.`,
  },

  {
    id: 'quan-phu-tai-quan-loc',
    title: 'Quan Phủ tại Quan Lộc',
    sao: ['Quan Phủ'],
    cung: ['Quan Lộc'],
    doUuTien: 78,
    tomTat: `🌟 **Quan Phủ cư Quan:** người **HỌC LUẬT, hành nghề LUẬT** — **THẨM
PHÁN, TRẠNG SƯ (LUẬT SƯ), công chứng viên, kiểm soát viên**.

🌟 Tư duy **sắc bén, giỏi lập luận**, công việc **tỉ mỉ, cẩn trọng, khắt khe**.

🌟 Công việc yêu cầu khả năng **soi xét, phán đoán, suy luận**, tính khắt khe
cao.

⚠️ Công việc có **tính cạnh tranh** và **hay bị cạnh tranh** trong nghề nghiệp.

⚠️ Công việc **dễ vướng PHÁP LUẬT, KIỆN TỤNG, KHỞI TỐ** → cần **hết sức cẩn
thận**, dễ bị **"đâm sau lưng"**, cạnh tranh không lành mạnh.`,
  },

  {
    id: 'quan-phu-tai-no-boc',
    title: 'Quan Phủ tại Nô Bộc',
    sao: ['Quan Phủ'],
    cung: ['Nô Bộc'],
    doUuTien: 70,
    tomTat: `⚠️ **Quan Phủ cư Nô (KHÔNG cát tinh):** dễ bị bạn bè, đồng nghiệp
**GHEN GHÉT, CHƠI XẤU, THƯA KIỆN**.

⚠️ Quan hệ đi kèm **đề phòng, xét nét, thiếu hài hòa**:
- Dễ bị **hiểu lầm, nói xấu**
- Bị người thân tín **trở mặt, chơi xấu sau lưng**

🌟 **+ Cát tinh:** quen nhiều người làm về **HÌNH PHÁP, PHÁP LUẬT** — gặp
được bạn tốt, có **năng lực phán xử**, có thể **giúp đỡ vấn đề pháp luật**.`,
  },

  {
    id: 'quan-phu-tai-thien-di',
    title: 'Quan Phủ tại Thiên Di',
    sao: ['Quan Phủ'],
    cung: ['Thiên Di'],
    doUuTien: 72,
    tomTat: `⚠️ **Quan Phủ cư Di:** ra ngoài dễ **dính THỊ PHI, KIỆN TỤNG** —
**"làm ơn mắc oán"**, bị **soi mói, dèm pha**.

🌟 **+ Cát tinh:** có thể **CỨU GIẢI** hoặc được giao **TRỌNG TRÁCH** yêu cầu
tính minh bạch cao:
- **Kiểm tra, thanh tra, giám sát**

→ Có **TÍN NHIỆM cao**, được tin tưởng giao việc quan trọng bên ngoài.`,
  },

  {
    id: 'quan-phu-tai-tat-ach',
    title: 'Quan Phủ tại Tật Ách - Tai họa pháp lý',
    sao: ['Quan Phủ'],
    cung: ['Tật Ách'],
    doUuTien: 75,
    tomTat: `⚠️ **Quan Phủ cư Tật:** mối nguy tiềm ẩn từ **PHÁP LÝ, KIỆN TỤNG,
THỊ PHI** → ảnh hưởng cả **sức khỏe tinh thần và thể chất**.

⚠️ **+ Thái Tuế / Hóa Kỵ / Liêm Trinh / Bạch Hổ / Thiên Hình:** nguy cơ rơi
vào **VÒNG LAO LÝ, TÙ TỘI** càng tăng.

⚠️ **+ Thái Tuế** đặc biệt: "**SUỐT ĐỜI MÒN MÒN NGỒI CỬA CÔNG MÔN**" — cuộc
đời gắn liền với tòa án.

🌟 **Hóa giải:** nếu làm nghề **LUẬT PHÁP** thì ảnh hưởng tiêu cực được giảm
nhẹ — nhờ **HIỂU BIẾT để thoát hiểm**, **chuyển bại thành thắng**.`,
  },

  {
    id: 'quan-phu-tai-tai-bach',
    title: 'Quan Phủ tại Tài Bạch',
    sao: ['Quan Phủ'],
    cung: ['Tài Bạch'],
    doUuTien: 75,
    tomTat: `🌟 **Quan Phủ cư Tài:** người **CẨN TRỌNG, NGUYÊN TẮC** trong tiền
nong — chi tiêu **có kiểm soát**, đòi hỏi **MINH BẠCH, RÕ RÀNG**.

🌟 **Phù hợp:**
- **Kiểm toán viên, kế toán pháp lý**
- **Cố vấn tài chính**
- **Luật sư chuyên về hợp đồng, tài sản**

⚠️ Người **kiếm tiền nghề KIỆN** (luật sư, hộ giá viên) hoặc đi cạnh tranh,
kiện tụng người khác để mình có tiền.

⚠️ **+ Hung sát:** dễ bị **KIỆN TỤNG, THỊ PHI vì TIỀN BẠC**, **bị phản bội vì
tiền** (lường gạt).`,
  },

  {
    id: 'quan-phu-tai-tu-tuc',
    title: 'Quan Phủ tại Tử Tức',
    sao: ['Quan Phủ'],
    cung: ['Tử Tức'],
    doUuTien: 72,
    tomTat: `🌟 **Quan Phủ cư Tử Tức + Cát tinh:** con cái **THÔNG MINH, TƯ DUY
SẮC BÉN, suy xét tốt** — dễ làm **NGHỀ THẨM PHÁN, ngành luật**, các lĩnh vực
đòi hỏi óc phân tích.

⚠️ **+ Sát bại tinh:** **KHẮC KHẨU với con cái**, xấu nữa có thể **TRANH TỤNG
với con**.

⚠️ Nếu **cung Phối cũng xấu:** có thể **TRANH GIÀNH QUYỀN NUÔI CON** giữa hai
vợ chồng (khi ly hôn).`,
  },

  {
    id: 'quan-phu-tai-phu-the',
    title: 'Quan Phủ tại Phu Thê',
    sao: ['Quan Phủ'],
    cung: ['Phu Thê'],
    doUuTien: 75,
    tomTat: `⚠️ **Quan Phủ cư Phu Thê:** đời sống hôn nhân **THIÊN VỀ LÝ TRÍ**,
ít tình cảm.

Người hôn phối:
- **NGUYÊN TẮC, GIỎI LÝ LUẬN**
- Có thể làm trong **ngành PHÁP LUẬT, hình pháp**

⚠️ Vì tính cách **quá rạch ròi và xét nét** → vợ chồng:
- Dễ **BẤT HÒA, soi xét nhau**
- **Tranh luận gay gắt** không ai nhường ai

⚠️ **+ Sát bại tinh:** Quan Phủ chính là **"QUAN TÒA"** — nơi hai người **ĐỆ
ĐƠN LY DỊ**.`,
  },

  {
    id: 'quan-phu-tai-huynh-de',
    title: 'Quan Phủ tại Huynh Đệ',
    sao: ['Quan Phủ'],
    cung: ['Huynh Đệ'],
    doUuTien: 70,
    tomTat: `⚠️ **Quan Phủ cư Huynh Đệ:** anh chị em **dễ BẤT HÒA, tranh giành
của cải mà KIỆN TỤNG nhau**.

⚡ Mỗi người có **tư duy riêng**, ít chịu nhường nhịn → tranh cãi:
- **Phân chia TÀI SẢN THỪA KẾ**
- **Đất đai tổ tiên**
- **Trách nhiệm phụng dưỡng cha mẹ**

🌟 **Có người làm LUẬT SƯ / THẨM PHÁN** → mâu thuẫn được **HÒA GIẢI**, giảm
tranh chấp nội bộ → **rất tốt**.`,
  },

  {
    id: 'quan-phu-thai-tue-da-ky',
    title: 'Quan Phủ + Thái Tuế + Đà La + Hóa Kỵ - Thị phi mất ngủ',
    sao: ['Quan Phủ'],
    ketHop: ['Thái Tuế', 'Đà La', 'Hóa Kỵ'],
    doUuTien: 65,
    tomTat: `⚠️ **Quan Phủ + Thái Tuế + Đà La + Hóa Kỵ:** chủ về **THỊ PHI, KIỆN
CÁO**, làm **mất ăn mất ngủ** vì **cạnh tranh hơn thiệt**.

→ Mâu thuẫn cũ được khơi mào → biến cố pháp lý → **kiện tụng dai dẳng**.

→ Lời nói thành gươm giáo, **một hành động nhỏ → hậu quả lớn về pháp lý**.`,
  },

  {
    id: 'quan-phu-hinh-khong-kiep',
    title: 'Quan Phủ + Thiên Hình + Không Kiếp - Thù oán mưu sát',
    sao: ['Quan Phủ'],
    ketHop: ['Thiên Hình', 'Địa Không', 'Địa Kiếp'],
    doUuTien: 65,
    tomTat: `⚠️ **Quan Phủ + Thiên Hình + Địa Không + Địa Kiếp:** thế cục của
**OAN TRÁI và ÂN OÁN cá nhân**.

→ Bản mệnh dễ:
- Bị **THÙ OÁN, KIỆN CÁO**
- Bị người khác **DỰNG CHUYỆN, MƯU SÁT VÌ THÙ**
- Hoặc chính bản thân **vì sinh hận mà gây họa**

→ Vướng vào **vòng xoáy trả thù**.`,
  },

  {
    id: 'quan-phu-sat-ho-hinh',
    title: 'Quan Phủ + Thất Sát + Bạch Hổ + Thiên Hình - Bị tù vu cáo',
    sao: ['Quan Phủ'],
    ketHop: ['Thất Sát', 'Bạch Hổ', 'Thiên Hình'],
    doUuTien: 65,
    tomTat: `⚠️ **Quan Phủ + Thất Sát + Bạch Hổ + Thiên Hình:** chủ về **TÙ TỘI,
ÁN PHẠT**.

→ Đương số dễ gặp **biến cố nghiêm trọng**:
- **BỊ VU KHỐNG, GÀI BẪY**
- **RƯỚC HỌA VÀO THÂN** do tin lầm người
- Mang họa pháp lý nghiêm trọng

→ Cách cục **CỰC HUNG** — cần đặc biệt thận trọng.`,
  },

  {
    id: 'quan-phu-liem-tang-ho-hinh-nguc-nan-dao',
    title: 'Quan Phủ + Liêm Trinh + Tang Môn + Bạch Hổ - "Hình ngục nan đào"',
    sao: ['Quan Phủ'],
    ketHop: ['Liêm Trinh', 'Tang Môn', 'Bạch Hổ'],
    doUuTien: 70,
    tomTat: `⚠️ **Quan Phủ + Liêm Trinh + Tang Môn + Bạch Hổ** = cách **"HÌNH
NGỤC NAN ĐÀO"** — **CẢNH LAO LÝ KHÓ TRÁNH**.

→ Điềm báo **CỰC XẤU**, ám chỉ:
- **TÙ TỘI**
- **TAI NẠN, TANG THƯƠNG**
- Do **bị PHẢN BỘI hoặc BÁO THÙ** mà có

⚠️ Một trong những cách cục cảnh báo **nguy hiểm NHẤT** liên quan đến pháp
lý trong tử vi — cần kiểm tra cẩn thận khi luận giải.`,
  },

  {
    id: 'quan-phu-vao-han',
    title: 'Quan Phủ vào hạn',
    sao: ['Quan Phủ'],
    doUuTien: 50,
    tomTat: `⚠️ **Hạn có Quan Phủ** (đặc biệt + **Thái Tuế / Đà La / Hóa Kỵ**):
vận trình rơi vào **VÒNG XOÁY TRANH CHẤP, KIỆN TỤNG DAI DẲNG**.

→ Đây là thời điểm:
- **Mâu thuẫn cũ ĐƯỢC KHƠI MÀO**
- Điều tưởng đã lắng xuống **TRỖI DẬY** thành **biến cố pháp lý**

⚠️ Sức ảnh hưởng có thể biến **lời nói thành gươm giáo** — chỉ một hành động
nhỏ cũng dẫn đến **hậu quả LỚN về pháp lý**.

💡 **Lời khuyên hạn:**
- **Cẩn trọng KÝ KẾT giấy tờ, hợp đồng**
- Tránh để **lòng tự ái, hơn thua** làm hỏng việc lớn
- "**Một câu nhịn, chín câu lành**" — biết lùi để giữ bình an`,
  },
];
