import type { DoanLuanGiai } from '../types';

/**
 * LUẬN GIẢI SAO HỒNG LOAN - Dâm Tinh Thủy, đối xung Thiên Hỷ
 * (Phần CHUNG của cặp Đào Hồng + Tam Minh Đào Hồng Hỷ đã viết tại dao-hoa.ts)
 * (File này chỉ chứa phần riêng của Hồng Loan + 11 cung)
 */
export const luanGiai_HongLoan: DoanLuanGiai[] = [
  // ============================================================
  // 1. Đặc tính riêng sao Hồng Loan
  // ============================================================
  {
    id: 'hong-loan-tinh-chat-chung',
    title: 'Hồng Loan - Đặc tính chung (đào hoa nội tâm)',
    sao: ['Hồng Loan'],
    doUuTien: 30,
    tomTat: `**Hồng Loan** — **Dâm Tinh** thuộc bộ **Tam Minh** (Đào - Hồng - Hỷ).

**Ngũ hành:** **Thủy**.

⚡ **LUÔN đối xung với Thiên Hỷ** (cách 6 cung).

**Đặc tính:** **xinh đẹp, đáng yêu, hỷ sự**, đem lại may mắn ở bất cứ đâu —
**duyên dáng, mềm mại, giàu cảm xúc**, sống thiên về **tình cảm**, yêu nghệ
thuật và cái đẹp.

⚡ **Khác biệt với Đào Hoa:**

| | Hồng Loan | Đào Hoa |
|---|---|---|
| **Hành** | **Thủy** (thâm trầm, nội tâm) | Mộc (đới Thủy) |
| **Hướng** | **Tâm hồn, nội tâm, tinh thần** | Vẻ bề ngoài, **thể xác** |
| **Cảm xúc** | **Ủy mị, dạt dào** trong cử chỉ, lời văn | Quyến rũ trực tiếp |
| **Sa đọa** | **Ít sa đọa hơn** | Dễ sa đọa hơn |

→ Hồng Loan tạo người **dễ xúc động, giàu tình cảm**, hướng tới tình cảm tâm
hồn hơn là dục vọng thể xác.`,

    chiTiet: `## Hướng dẫn an sao Hồng Loan

Hồng Loan an theo **địa chi của năm sinh**:
- Lấy cung **Mão** làm mốc cho năm **Tý**
- Chạy **NGƯỢC chiều kim đồng hồ** tới địa chi của năm sinh
- → Sẽ xác định được vị trí Hồng Loan

(Thiên Hỷ luôn an đối xung với Hồng Loan — cách 6 cung)

## Vai trò trong tử vi

- **Chi phối mạnh** các mối quan hệ tình cảm, hôn nhân
- Ảnh hưởng **gián tiếp** đến công việc và tài lộc
- Mang lại **hỷ sự** (tin vui, cưới hỏi, sinh nở)`,
  },

  // ============================================================
  // 2. Vị trí Hồng Loan tại Tứ Chính / Tứ Mộ / Tứ Sinh
  // ============================================================
  {
    id: 'hong-loan-vi-tri-tu-chinh-mo-sinh',
    title: 'Hồng Loan - Vị trí Tứ Chính / Tứ Mộ / Tứ Sinh',
    sao: ['Hồng Loan'],
    doUuTien: 50,
    tomTat: `⚡ **Vị trí Hồng Loan ảnh hưởng RẤT MẠNH đến ý nghĩa**:

🌟 **TỨ CHÍNH (Tý / Ngọ / Mão / Dậu) — TỐT NHẤT:**
- Luôn **đồng cung hoặc hội chiếu với Đào Hoa** → tạo **Tam Minh Đào Hồng Hỷ**
- Đem lại **may mắn, niềm vui, nhân duyên**
- **Đặc biệt đẹp khi có Âm Dương đi cùng**

⚠️ **TỨ MỘ (Thìn / Tuất / Sửu / Mùi) — XẤU NHẤT:**
- Đặc điểm ngũ hành **Thổ tương khắc Thủy** → Hồng Loan trở thành sao **lắm
  duyên mà kém phận**
- **"Lắm mối tối nằm không"**, tình duyên dễ **dang dở, đứt đoạn**
- Người hay **buồn bã, âu sầu vì tình yêu**

⚡ **TỨ SINH (Dần / Thân / Tỵ / Hợi) — TÂM LINH:**
- Luôn đi cùng **Thiên Không / Cô Quả** (hoặc hội chiếu)
- Chủ về **"sắc sắc không không"** — tình ái có hay không đều như một
- Hướng tới **tu hành, mong cầu tỉnh thức** — bài học buông bỏ`,
  },

  // ============================================================
  // 3. Hồng Loan + Thiên Hỷ - đối xung
  // ============================================================
  {
    id: 'hong-loan-thien-hy-doi-xung',
    title: 'Hồng Loan + Thiên Hỷ - Bộ đôi đối xung mang hỷ sự',
    sao: ['Hồng Loan', 'Thiên Hỷ'],
    doUuTien: 55,
    tomTat: `🌟 **Hồng Loan + Thiên Hỷ** — bộ đôi **LUÔN đối xung** (cách 6 cung)
mang lại:
- **Vui vẻ, cưới hỏi thuận lợi**
- Đi đâu cũng có **tiếng cười**
- **Hỷ sự** (tin vui, sinh nở, lập gia thất)

→ Hồng Loan và Thiên Hỷ cùng thuộc bộ **Tam Minh** (cùng với Đào Hoa) →
chuyên chủ về niềm vui, hỉ sự.

⚠️ **Hạn đến Hồng Loan + Thiên Hỷ với người TUỔI GIÀ**: dễ **về với tổ tiên**
(qua đời).`,
  },

  // ============================================================
  // 4. Hồng Loan tại Mệnh
  // ============================================================
  {
    id: 'hong-loan-tai-menh',
    title: 'Hồng Loan tại Mệnh',
    sao: ['Hồng Loan'],
    cung: ['Mệnh'],
    doUuTien: 80,
    tomTat: `🌟 **Hồng Loan thủ Mệnh:** người **đẹp đẽ, khéo léo về chân tay**,
**nhẹ nhàng, thướt tha, duyên dáng**, có nét **thu hút, quyến rũ**.

**Tướng mạo:**
- **Da trắng**, **có da có thịt, béo tốt, đẫy đà**
- **Có khiếu ăn mặc, gu thẩm mỹ cao**
- Ra ngoài thích mặc đồ **diêm dúa, nhiều màu sắc, hoa hòe**

(Xem entry chuyên biệt theo giới tính cho chi tiết tướng mạo + hôn nhân.)

**Tính tình:** ưa tìm cảm xúc bên trong tâm hồn, **không ham vật chất**, tính
**như nghệ sĩ**.

⚠️ **Không chịu được cô đơn** — phải hay đi ra ngoài, không thể ngồi một chỗ.

🌟 **Năng khiếu thiên bẩm:**
- **Hội họa, âm nhạc** rất tốt
- **Ẩm thực, làm đẹp**
- Khả năng cảm nhận **hình khối, hình thể**`,

    chiTiet: `## Sự nghiệp - lập thân từ sớm

🌟 Hồng Loan tại Mệnh giúp đương số **lập thân từ sớm** — bước vào đời khá
sớm, nhanh chóng có việc làm và sớm hình thành định hướng nghề nghiệp.

🌟 **Con đường công danh được nâng đỡ bởi nhân duyên**, đặc biệt là từ **người
khác phái** → quý nhân xuất hiện đúng lúc, mở ra cơ hội.

→ Sự nghiệp và tình cảm nhiều khi đan xen.

🌟 **Phù hợp:** thời trang, làm đẹp, kiến trúc, tổ chức sự kiện, nghệ thuật
biểu diễn.

## Hôn nhân - phức tạp nhất

(Xem entry chuyên biệt theo giới tính cho đặc điểm hôn nhân.)

⚠️ Nếu **không có Vũ Khúc / Thiên Hình** kèm theo, hôn nhân dễ **"sớm nở tối
tàn"**. Hôn nhân bắt đầu từ cảm xúc bốc đồng thường kết thúc nhanh chóng.

## Sức khỏe - hành Thủy

⚠️ Cần lưu ý các bệnh:
- **Máu huyết, hệ tiêu hóa** (do hành Thủy)
- Khi **cảm xúc bất ổn** → sức khỏe cũng dễ suy yếu

⚠️ **Nhiều tai họa phát sinh từ TÂM BỆNH**:
- **Cú sốc tình cảm, thất tình** → thể trạng suy sụp nhanh
- **+ Thai / Mộc Dục:** bệnh **lây nhiễm qua đường tình cảm**

⚠️ **+ Hóa Kỵ / Địa Kiếp:** đề phòng **TAI NẠN SÔNG NƯỚC** — chuyến đi xa, du
lịch, tắm biển, đi thuyền cần thận trọng.

## Tài lộc

🌟 Khả năng kiếm tiền nhờ **nhan sắc và tài năng** — từ trẻ đã có thu nhập
nhờ nghệ thuật, biểu diễn, thẩm mỹ.

⚠️ Tiền kiếm được **không tích lũy nhiều** vì có xu hướng chi tiêu cho:
- Hưởng thụ, mua sắm, làm đẹp
- Thú vui tinh thần
- Nhân tình

→ Với chủ mệnh, **tiền là để nuôi dưỡng cảm xúc và phong cách sống**.

## Lời khuyên

1. **Học cách giữ lý trí trong tình yêu** — đừng để rung động thoáng qua
   quyết định bước ngoặt lớn (hôn nhân, sự nghiệp)
2. **Lấy tài năng làm chỗ dựa** — dùng nghệ thuật, thẩm mỹ, sáng tạo để xây
   dựng sự nghiệp
3. **Cẩn trọng sông nước** và môi trường ẩm thấp — yếu tố dễ kích hoạt hạn xấu
4. Khi **thân tâm đều vững**, Hồng Loan mới thực sự phát huy được vẻ đẹp và
   phúc khí`,
  },

  {
    id: 'hong-loan-tai-menh-nu',
    title: 'Hồng Loan tại Mệnh - Nữ mệnh',
    sao: ['Hồng Loan'],
    cung: ['Mệnh'],
    gioiTinh: 'Nữ',
    doUuTien: 75,
    tomTat: `🌟 **Hồng Loan tại Mệnh - Nữ mệnh:**

**Tướng mạo:** thướt tha, mềm mại, điệu đà — sáng một bộ chiều một bộ, **thay
đổi mốt liên tục**.

**Hôn nhân:** rất đào hoa, nhiều người theo đuổi nhưng dễ rơi vào **"lắm mối
tối nằm không"**. **Nhạy cảm dễ xúc động** → dễ yêu cũng dễ đau khổ vì yêu.`,
  },

  {
    id: 'hong-loan-tai-menh-nam',
    title: 'Hồng Loan tại Mệnh - Nam mệnh',
    sao: ['Hồng Loan'],
    cung: ['Mệnh'],
    gioiTinh: 'Nam',
    doUuTien: 75,
    tomTat: `🌟 **Hồng Loan tại Mệnh - Nam mệnh:**

Sức hút tự nhiên, **sinh lực mạnh**, nói chuyện duyên dáng → dễ chiếm cảm
tình khác phái.

⚠️ Khó giữ chung thủy tuyệt đối, dễ vướng quan hệ ngoài lề.`,
  },

  // ============================================================
  // 5. Hồng Loan tại Phụ Mẫu
  // ============================================================
  {
    id: 'hong-loan-tai-phu-mau',
    title: 'Hồng Loan tại Phụ Mẫu',
    sao: ['Hồng Loan'],
    cung: ['Phụ Mẫu'],
    doUuTien: 75,
    tomTat: `**Hồng Loan cư Phụ Mẫu:** cha mẹ là người **đào hoa**, thu hút khác
giới, **ra ngoài nổi tiếng**.

🌟 Cha mẹ **lãng mạn**, thích **văn thơ, âm nhạc**, có thiên hướng **yêu nghệ
thuật và sống tình cảm**, dễ là người **lấy nhau sớm**.

🌟 **+ Cát tinh:** cha mẹ hiền hòa, có uy tín, sống tình cảm, đương số được
sinh trưởng trong môi trường văn minh, tình cảm → có nền tảng để phát triển
học vấn và sự nghiệp.

⚠️ **+ Không Kiếp / Kình Đà:** duyên tình của bố mẹ **dang dở** hoặc gặp **trắc
trở về hôn nhân**, có thể **ly hôn hoặc góa bụa**.

⚡ **Hồng + Thiên Không + Cô + Quả:** giảm nhân duyên cha mẹ — về sau **người
còn lại thường thích tu tập**, tìm hiểu tâm linh.`,
  },

  // ============================================================
  // 6. Hồng Loan tại Phúc Đức
  // ============================================================
  {
    id: 'hong-loan-tai-phuc-duc',
    title: 'Hồng Loan tại Phúc Đức',
    sao: ['Hồng Loan'],
    cung: ['Phúc Đức'],
    doUuTien: 75,
    tomTat: `**Hồng Loan cư Phúc Đức:**

🌟 **Đào Hồng cư Phúc:** các cụ ngày xưa **đào hoa, nhiều vợ**; họ hàng có
nhiều người **thành danh, thành đạt, may mắn**.

🌟 Trong dòng họ nhiều người **sống tình cảm, nhẹ nhàng, tốt tính**.

🌟 Đương số **hay gặp quý nhân là người KHÁC GIỚI** giúp đỡ, hỗ trợ.

⚡ **Mồ mả tổ tiên** thường gần nơi **ẩm ướt, sông hồ** (hành Thủy).

⚠️ **+ Hung sát:** họ hàng vướng duyên tình trắc trở, góa bụa, tang tóc, tranh
chấp đất đai/mồ mả. Đương số khó tránh nghiệp duyên ràng buộc.

(Xem entry chuyên biệt cho cách Hồng Loan cư Phúc Đức tại Tứ Mộ.)`,
  },

  {
    id: 'hong-loan-tai-phuc-duc-tu-mo',
    title: '⚠️ Hồng Loan tại Phúc Đức ở Tứ Mộ - Nhiều người góa chồng',
    sao: ['Hồng Loan'],
    cung: ['Phúc Đức'],
    chi: ['Thìn', 'Tuất', 'Sửu', 'Mùi'],
    doUuTien: 70,
    tomTat: `⚠️ **Hồng Loan cư Phúc Đức tại Tứ Mộ (Thìn / Tuất / Sửu / Mùi):**

Trong dòng họ, các bà thọ hơn các ông, **nhiều người góa chồng** — đặc trưng
do tính chất Thổ khắc Thủy của Hồng Loan tại các cung Thổ.`,
  },

  // ============================================================
  // 7. Hồng Loan tại Điền Trạch
  // ============================================================
  {
    id: 'hong-loan-tai-dien-trach',
    title: 'Hồng Loan tại Điền Trạch - Đào Hồng cư Điền là thượng cách',
    sao: ['Hồng Loan'],
    cung: ['Điền Trạch'],
    doUuTien: 80,
    tomTat: `🌟 **Đào Hồng đóng cung Điền Trạch = THƯỢNG CÁCH** trong tử vi.

→ Đào Hồng = may mắn, sinh trưởng, duyên tình → đương số:
- **Sớm nhà sớm cửa**
- Nhà thường **trang trí ĐẸP**
- Nhà cửa có **trang viên, thư phòng**

⚡ **Đặc điểm nhà cửa:**
- Màu sắc **sặc sỡ**, **nhiều tranh ảnh**, **lắm đồ trang trí**
- Không gian sống đậm chất nghệ thuật

⚠️ **Quan hệ hàng xóm:** dễ **nảy sinh tình cảm, nhân duyên** với hàng xóm
xung quanh — thường thân mật với hàng xóm **khác giới**. Hàng xóm dễ có người
**góa chồng**.

⚠️ **+ Sao xấu:** nhà cửa dễ **cháy nổ, hư hỏng**, vướng tranh chấp hoặc ở
gần chốn ồn ào, thị phi → bản mệnh dễ vướng nghiệp duyên với người xung quanh.`,
  },

  // ============================================================
  // 8. Hồng Loan tại Quan Lộc
  // ============================================================
  {
    id: 'hong-loan-tai-quan-loc',
    title: 'Hồng Loan tại Quan Lộc - Hoa sớm nở ở quan trường',
    sao: ['Hồng Loan'],
    cung: ['Quan Lộc'],
    doUuTien: 80,
    tomTat: `🌟 **Hồng Loan cư Quan Lộc** = **"hoa sớm nở ở quan trường sự nghiệp"**:
- **Ra đời sớm**, sớm có công ăn việc làm, **lập thân từ trẻ**
- Có **may mắn về công danh** do **người khác giới mang đến**, được giúp đỡ
  hỗ trợ

🌟 **+ Đào Hoa + Thiên Hỷ** (Tam Minh): công danh **lớn lao, dễ dàng**, **thi
cử đỗ đầu**, đậu danh.

🌟 **Phù hợp công việc liên quan tới CẢM XÚC, NGHỆ THUẬT, VĂN THƠ**:
- **Đào Hồng:** làm đẹp, nghệ thuật, **kiến trúc, tổ chức sự kiện**...
- ⚡ **Cơ + Hồng + Hình + Phi Liêm:** **may dệt, cắt tóc**

⚠️ **+ Sát tinh vây hãm:** sự nghiệp **"sớm nở tối tàn"**, có tài nhưng vướng
**thị phi**, tình cảm xen vào công việc. Phải lao động tay chân, ít được
trọng dụng.`,
  },

  // ============================================================
  // 9. Hồng Loan tại Nô Bộc
  // ============================================================
  {
    id: 'hong-loan-tai-no-boc',
    title: 'Hồng Loan tại Nô Bộc',
    sao: ['Hồng Loan'],
    cung: ['Nô Bộc'],
    doUuTien: 75,
    tomTat: `**Hồng Loan cư Nô:** người có **sức thu hút quyến rũ khác phái**,
được bè bạn, đồng nghiệp **thương mến**, **nhiều bạn bè nữ giới**.

🌟 **+ Tử Vi / Thiên Phủ / Thái Dương:** bè bạn **nể phục, kính trọng** vì sự
**đoan chính, ngay thẳng, nết hạnh** của mình.

⚠️ **Đặc trưng:** Hồng Loan = duyên → đương số rất **lắm duyên**, dễ có **nhân
tình bên ngoài**.

⚠️ **Cung Phu/Thê có sao xấu** (Hình / Kỵ / Tả Hữu / Tham Lang): rất dễ **hai
đời vợ chồng**.

⚠️ **+ Tham Lang / Thiên Hình / Hóa Kỵ:** nguy cơ tình cảm rối ren, **nhân
tình ngoài luồng** càng cao. Bạn bè lợi dụng, đồng nghiệp ganh ghét.`,
  },

  // ============================================================
  // 10. Hồng Loan tại Thiên Di
  // ============================================================
  {
    id: 'hong-loan-tai-thien-di',
    title: 'Hồng Loan tại Thiên Di',
    sao: ['Hồng Loan'],
    cung: ['Thiên Di'],
    doUuTien: 78,
    tomTat: `🌟 **Hồng Loan cư Thiên Di:** ra ngoài **giao thiệp dễ thuyết phục**
được người khác, có **nhiều lợi lộc do người khác phái mang lại**.

🌟 Ra ngoài dễ **thu hút khác giới**, được nhiều người **hâm mộ, nổi tiếng**.

🌟 **+ Cát tinh:** chủ mệnh được nhiều người quý mến, **kết giao bạn tốt**, hay
**gặp quý nhân khác giới** nâng đỡ. Đường đời suôn sẻ hơn khi **xa quê lập
nghiệp** — sự nghiệp nở rộ nhờ xã giao, công chúng. Đặc biệt thuận lợi:
**nghệ thuật, ngoại giao, truyền thông**.

⚠️ **+ Sao xấu:** vẫn có sức hút khác giới nhưng kéo theo **nhiều thị phi, rắc
rối tình cảm**. Đời sống xã hội biến động — khi rực rỡ, khi lại **sa vào tai
tiếng** gây hệ lụy lớn cho sự nghiệp.`,
  },

  // ============================================================
  // 11. Hồng Loan tại Tật Ách
  // ============================================================
  {
    id: 'hong-loan-tai-tat-ach',
    title: 'Hồng Loan tại Tật Ách - Tai họa từ tâm bệnh',
    sao: ['Hồng Loan'],
    cung: ['Tật Ách'],
    doUuTien: 78,
    tomTat: `⚡ **Hồng Loan cư Tật Ách:** dễ gặp **tai họa do tình ái gây ra** —
đương số chiều chuộng nhân tình của mình.

⚠️ **Đặc trưng:** tai họa bệnh tật thường phát sinh từ **bị tổn thương tâm hồn,
THẤT TÌNH** mà ra → **TÂM BỆNH**, trầm uất.

⚠️ **Bệnh thường gặp** (do hành Thủy):
- **Đường tiêu hóa**
- **Máu huyết**

⚠️ **+ Thai / Mộc Dục:** rất dễ mắc bệnh liên quan **đường tình dục**.

🌟 **+ Cát tinh:** ít gặp bệnh nặng, đa phần bệnh nhẹ dễ chữa, **gặp được
thầy thuốc giỏi**. Tổn thương tình cảm cũng chóng lành — biết tìm đến **nghệ
thuật, tôn giáo** để cân bằng.

⚠️ **+ Sát tinh vây hãm:** ốm đau không dứt, **bệnh mãn tính** về máu huyết,
tiêu hóa, sinh dục. Cú sốc tình cảm để lại **vết thương lâu dài**, dễ **trầm
uất**.`,
  },

  // ============================================================
  // 12. Hồng Loan tại Tài Bạch
  // ============================================================
  {
    id: 'hong-loan-tai-tai-bach',
    title: 'Hồng Loan tại Tài Bạch',
    sao: ['Hồng Loan'],
    cung: ['Tài Bạch'],
    doUuTien: 78,
    tomTat: `🌟 **Đào Hồng cư Tài:** kiếm tiền **từ sớm**, **nhiều tài**, kiếm
tiền **dễ dàng thuận lợi**, **gặp nhiều may mắn**.

⚡ **Đặc trưng tiêu tiền:** hay tốn tiền cho:
- **Làm đẹp**
- Mua vui cho **tâm hồn**
- **Nhân tình**

(Xem entry chuyên biệt cho nữ mệnh khi đi với sao xấu.)

🌟 **+ Cát tinh:** cuộc sống **an nhàn**, cả đời **may mắn**.

🌟 **+ Cát tinh + nghề nghệ thuật/thời trang/làm đẹp/giải trí/dịch vụ:** rất
**phát đạt**, hay **gặp quý nhân khác giới** giúp đỡ tiền bạc.

⚠️ **+ Sát tinh:** tiền đến nhanh nhưng đi cũng nhanh — **hao tổn** từ tình
cảm, ăn chơi, làm đẹp, nhân tình. Dễ vướng **nợ nần, tranh chấp, bị bạn lợi
dụng**.`,
  },

  {
    id: 'hong-loan-tai-tai-bach-nu-sao-xau',
    title: '⚠️ Hồng Loan tại Tài Bạch + Sao xấu - Nữ mệnh dùng nhan sắc',
    sao: ['Hồng Loan'],
    cung: ['Tài Bạch'],
    gioiTinh: 'Nữ',
    doUuTien: 70,
    tomTat: `⚠️ **Hồng Loan cư Tài Bạch + nhiều sao xấu - Nữ mệnh:**

Dễ kiếm tiền **dựa vào nhan sắc** — lúc này **không đẹp cho lắm**.`,
  },

  // ============================================================
  // 13. Hồng Loan tại Tử Tức - Sinh con Khôi Việt Cái Hồng
  // ============================================================
  {
    id: 'hong-loan-tai-tu-tuc',
    title: 'Hồng Loan tại Tử Tức - Cách "Sinh con Khôi Việt Cái Hồng"',
    sao: ['Hồng Loan'],
    cung: ['Tử Tức'],
    doUuTien: 78,
    tomTat: `🌟 **Hồng Loan cư Tử Tức:** dễ **đẻ NHIỀU CON GÁI hơn con trai**, dễ
có **CON GÁI ĐẦU LÒNG**.

Con cái:
- **Xinh đẹp**, có **tài năng từ lúc trẻ**
- Khả năng về **âm nhạc, nghệ thuật, hình khối**
- **Tình cảm, thướt tha, tâm hồn nghệ sĩ**

🌟 **Đào Hồng cư Tử:** con cái dễ **thành danh thành đạt**.

🌟 **+ Khôi Việt:** con cái **ngoan ngoãn, học giỏi, may mắn thi cử** — phú nói:

> **"Sinh con Khôi Việt - Cái - Hồng**
> **Trai tận cửu trùng, Nữ tác cung phi"**
> (Trai làm tới chức cao trong triều đình, Nữ làm hoàng phi)

⚠️ **+ Sát tinh vây hãm:** trắc trở chuyện sinh nở, con khó nuôi, sức khỏe
yếu. Duyên tình con cái phức tạp. **Phá cách nặng:** hiếm muộn, con khó cầu.`,
  },

  // ============================================================
  // 14. Hồng Loan tại Phu Thê
  // ============================================================
  {
    id: 'hong-loan-tai-phu-the',
    title: 'Hồng Loan tại Phu Thê',
    sao: ['Hồng Loan'],
    cung: ['Phu Thê'],
    doUuTien: 80,
    tomTat: `🌟 **Hồng Loan cư Phu Thê:** người hôn phối **có nhan sắc, quyến rũ,
tháo vát**, hoàn cảnh gia đình **khá giả**, vợ chồng **xứng lứa vừa đôi**, lấy
nhau ở **phương xa**.

⚡ Người hôn phối **đa tình, lãng mạn**, hoặc làm các nghề liên quan đến **nghệ
thuật, thời trang**.

⚠️ **+ Sát bại tinh:** người hôn phối có **ngoại tình** hoặc **chấp nối**,
**nhiều lần lập gia đình**.

🌟 **+ Quang Quý / Khôi Việt:** **GIẢM tính đào hoa** của người hôn phối →
vợ chồng **chiều chuộng nhau, hợp nhau**.

⚠️ **+ Không Kiếp / Hóa Kỵ / Kình Đà / Cô Quả / Tuần Triệt:** **nhanh cưới
nhanh bỏ** — vợ chồng hay ghen, lúc yêu thì đòi cưới, **cưới về thì đòi bỏ
nhau ngay được**.

⚠️ **+ Hỏa / Linh:** nguy cơ đổ vỡ vì ghen tuông, thiếu tin tưởng. Vợ/chồng
**phóng túng**, dễ có **quan hệ ngoài luồng**.`,
  },

  // ============================================================
  // 15. Hồng Loan tại Huynh Đệ
  // ============================================================
  {
    id: 'hong-loan-tai-huynh-de',
    title: 'Hồng Loan tại Huynh Đệ',
    sao: ['Hồng Loan'],
    cung: ['Huynh Đệ'],
    doUuTien: 75,
    tomTat: `**Hồng Loan cư Huynh Đệ:** **nhiều chị em gái**, anh chị em làm ăn
**xa nhà**.

🌟 **+ Cát tinh:** anh chị em **giỏi giang**, có năng khiếu **văn chương, nghệ
thuật**. Quan hệ **"máu chảy ruột mềm"**, gắn bó, nâng đỡ nhau khi cần.

⚠️ **+ Hồng + Riêu + Không Kiếp:** có **chị em gái BỎ NHÀ THEO TRAI**.

⚠️ **+ Sát tinh nhiều:** tình cảm gia đình **bất hòa**, ít gắn bó. Có người
**lận đận tình duyên**, thậm chí bỏ nhà đi theo tiếng gọi tình cảm.

⚠️ **+ Không Kiếp / Đà La:** anh em có nguy cơ **ly tán** hoặc **yểu mệnh**.`,
  },

  // ============================================================
  // 16. Bộ sao chế giảm tính đa tình của Hồng Loan
  // ============================================================
  {
    id: 'hong-loan-sao-che-giam',
    title: 'Bộ sao chế giảm tính đa tình của Hồng Loan',
    sao: ['Hồng Loan'],
    doUuTien: 45,
    tomTat: `🌟 **Bộ sao GIẢM tính đa tình, lãng mạn** của Hồng Loan:

**Đồng cung với:**
- **Thiên Hình**
- **Vũ Khúc**
- **Song Hao** (Đại Hao + Tiểu Hao)
- **Thái Dương sáng sủa**
- **Thiên Khôi** hoặc **Thiên Việt**

**Vị trí (vòng Tràng Sinh):**
- Hồng Loan nằm trên đất **Suy / Tử / Tuyệt** → giảm tính đa tình

🌟 **+ Khôi Việt / Thái Dương sáng** còn hướng đương số vào sự **chính chuyên**,
giúp tài năng nghệ thuật **phát tiết rực rỡ** và có **danh tiếng bền vững**.`,
  },

  // ============================================================
  // 17. Hồng Loan + sát tinh - thị phi tình ái
  // ============================================================
  {
    id: 'hong-loan-sat-tinh-thi-phi-tinh-ai',
    title: 'Hồng Loan + Sát tinh - Thị phi tình ái, hôn nhân sóng gió',
    sao: ['Hồng Loan'],
    doUuTien: 55,
    tomTat: `⚠️ **Hồng Loan + Sát tinh** — các tình huống xấu:

**1. Vướng thị phi tình ái:**
- ⚠️ **+ Thiên Riêu / Thiên Y:** quan hệ vợ chồng **lục đục** do **ghen tuông
  và đa nghi**, tình huống **không minh bạch** trong tình cảm

**2. Hôn nhân sóng gió:**
- ⚠️ **+ Kình Dương / Đà La:** vợ chồng **chung sống không thuận**, **tranh
  cãi triền miên** do tính tình **nóng nảy thiếu kiên nhẫn**

**3. Sa đọa vào tệ nạn:**
- ⚠️ **+ Đào Hoa + Hỏa Tinh + Linh Tinh:** **kích thích dục vọng tăng cao** →
  sa ngã vào **thú vui nhất thời**, đánh mất tỉnh táo. Hệ quả: **danh tiếng và
  quan hệ xã hội bị ảnh hưởng nghiêm trọng**, nhiều rắc rối tai tiếng khó gột rửa`,
  },

  // ============================================================
  // 18. Hồng Loan + Hóa Kỵ / Không Kiếp - thủy nạn
  // ============================================================
  {
    id: 'hong-loan-hoa-ky-khong-kiep',
    title: 'Hồng Loan + Hóa Kỵ / Không Kiếp - Đề phòng thủy nạn',
    sao: ['Hồng Loan'],
    ketHop: ['Hóa Kỵ', 'Địa Không', 'Địa Kiếp'],
    doUuTien: 60,
    tomTat: `⚠️ **Hồng Loan + Hóa Kỵ / Không Kiếp:** đề phòng **TAI NẠN ĐUỐI
NƯỚC**.

(Vì Hồng Loan thuộc **hành Thủy** + sao xấu hành Thủy → kích hoạt thủy nạn)

⚠️ Cẩn trọng:
- **Sông nước**, **tắm biển**
- Du lịch **đi thuyền**
- Môi trường **ẩm thấp**

⚠️ **Đào Hồng + Không Kiếp + Bệnh Phù:** mắc **bệnh phong tình**.

(Xem entry chuyên biệt cho Đào Hồng tại Mệnh + Thiên Không / Địa Không.)`,
  },

  {
    id: 'hong-loan-tai-menh-thien-khong-can-dong',
    title: '⚠️ Đào Hồng tại Mệnh + Thiên Không / Địa Không - Căn đồng số lính',
    sao: ['Đào Hoa', 'Hồng Loan'],
    cung: ['Mệnh'],
    ketHop: ['Thiên Không', 'Địa Không'],
    doUuTien: 65,
    tomTat: `⚠️ **Đào Hồng cư Mệnh + Thiên Không / Địa Không:**

- Rất dễ là người **dính dáng đến căn đồng số lính**
- Tình duyên lận đận
- **Khổ vì tình**`,
  },

  // ============================================================
  // 19. Hồng Loan vào hạn
  // ============================================================
  {
    id: 'hong-loan-vao-han',
    title: 'Hồng Loan vào hạn',
    sao: ['Hồng Loan'],
    doUuTien: 50,
    tomTat: `🌟 **Hạn có Hồng Loan + Đào Hoa + Thiên Hỷ + Hỷ Thần + Thái Dương +
Thái Âm + Thiên Đồng + Tham Lang:** hạn có **HỶ SỰ**, **THÀNH GIA THẤT**.

🌟 **+ Đào Hoa + Thiên Hỷ + Hỷ Thần** đồng cung: dễ **cưới hỏi, lập gia thất**
hoặc gặp **ý chung nhân**.

⚠️ **+ Địa Kiếp:** hạn **duyên nghiệp** — **long đong, trắc trở** trong tình cảm.

⚠️ **+ Cự Môn / Hóa Kỵ:** hạn **tơ duyên rắc rối**, **đến rồi lại đi liên tục**.

⚠️ **+ Hóa Kỵ / Không Kiếp:** đề phòng **HẠN ĐUỐI NƯỚC**.

⚠️ **+ Thiên Hỷ với người TUỔI GIÀ:** dễ **về với tổ tiên**.`,
  },
];
