import type { DoanLuanGiai } from '../types';

/**
 * LUẬN GIẢI SAO BÁC SĨ - Cát Tinh, Thủy, đứng đầu vòng Bác Sĩ
 * (Bác Sĩ LUÔN đồng cung Lộc Tồn - phần "Lộc Tồn + Bác Sĩ thủ Mệnh" đã viết tại loc-ton.ts)
 */
export const luanGiai_BacSi: DoanLuanGiai[] = [
  // ============================================================
  // 1. Đặc tính chung sao Bác Sĩ
  // ============================================================
  {
    id: 'bac-si-tinh-chat-chung',
    title: 'Bác Sĩ - Đặc tính chung',
    sao: ['Bác Sĩ'],
    doUuTien: 30,
    tomTat: `**Bác Sĩ** — phụ tinh **Cát Tinh**, **đứng ĐẦU vòng Bác Sĩ** (12 sao
theo can năm sinh).

**Ngũ hành:** **Thủy**.

**Đặc tính:** **sáng suốt, từ bi nhân hậu, khéo léo, văn chương, học hành, thi
cử**.

**LUÔN đồng cung với Lộc Tồn** — vì thế nhiều người gọi nhầm là "vòng Lộc
Tồn", thực chất phải là **"vòng Bác Sĩ"** mới đúng (Bác Sĩ là sao đứng đầu).

**So với Hóa Khoa:** Bác Sĩ có đặc tính **giống Hóa Khoa** (chủ học hành,
giải cứu bệnh tật) nhưng **KHÔNG MẠNH bằng** Hóa Khoa.

**Ý nghĩa chính:**
- Người **thông minh, suy nghĩ kín kẽ, chu đáo, sâu sắc**
- **Ngôn ngữ linh hoạt**, hoạt bát nhân hậu, khoan hòa
- **Lợi ích cho thi cử học hành** và **giải trừ bệnh tật**
- Có **khả năng cứu chữa bệnh nhẹ**`,

    chiTiet: `## Vòng Bác Sĩ - 12 sao theo thứ tự

(An theo Can năm sinh — chiều thuận với Dương Nam/Âm Nữ, nghịch với Âm Nam/Dương Nữ)

| # | Sao | Tính chất |
|---|---|---|
| 0 | **Bác Sĩ + Lộc Tồn** đồng cung | Cát - học thức, tài lộc |
| 1 | **Lực Sĩ** | Cát - sức lực, lao động |
| 2 | **Thanh Long** | Cát - hỷ khí, tin vui |
| 3 | **Tiểu Hao** | Hung nhẹ - hao tán nhỏ |
| 4 | **Tướng Quân** | Cát - quyền uy, võ tướng |
| 5 | **Tấu Thư** | Cát - văn chương, học thức |
| 6 | **Phi Liêm** | Hung - thị phi, tin đồn |
| 7 | **Hỷ Thần** | Cát - hỷ sự, vui mừng |
| 8 | **Bệnh Phù** | Hung - bệnh tật |
| 9 | **Đại Hao** | Hung - hao tán lớn |
| 10 | **Phục Binh** | Hung - phục kích, ám hại |
| 11 | **Quan Phủ** | Hung - kiện tụng, pháp đình |

## Khả năng đặc biệt

Hóa giải một số **hung tinh về bệnh tật** — gặp Bác Sĩ thì bệnh thường
**mau bình phục**.

## Ảnh hưởng tổng quan

- **Sức khỏe tốt**, ít bệnh tật
- Có **duyên với ngành y, dược, giáo dục**
- **Hài hòa**, giảm biến động trong hôn nhân
- **Tài lộc ổn định**, có mức độ kiểm soát`,
  },

  // ============================================================
  // 2. Bác Sĩ tại Mệnh
  // ============================================================
  {
    id: 'bac-si-tai-menh',
    title: 'Bác Sĩ tại Mệnh - Thông minh học hành đỗ đạt',
    sao: ['Bác Sĩ'],
    cung: ['Mệnh'],
    doUuTien: 80,
    tomTat: `**Bác Sĩ thủ Mệnh:** người **từ bi sáng suốt, thông minh**, hiểu
biết **sâu rộng**, suy xét **kín kẽ, khôn ngoan**, **khéo léo trong ngôn từ**.

**Tính cách:**
- Phong thái **điềm đạm**, biết **cân nhắc trước sau**
- **Cực kỳ khéo léo** trong giao tiếp, ứng xử
- **Dễ tạo thiện cảm**, được mọi người **quý trọng và tín nhiệm**

**Có học thức cao**, **TĂNG THỌ TRƯỜNG**.

**Khả năng chữa bệnh nhẹ** — biết cách chăm sóc bản thân, lối sống khoa học,
**phòng bệnh hơn chữa bệnh**.

**Bác Sĩ LUÔN đồng cung Lộc Tồn**: thừa hưởng vận khí **no ấm, ổn định**:
- Cuộc sống **vững vàng, ít thiếu thốn**
- Dễ được **thừa hưởng tiền bạc/tài sản** từ gia đình

**Cung Mệnh luôn bị Kình Dương + Đà La giáp** (do Lộc Tồn): tuổi thơ cô
độc, phải tự lập từ sớm. Xem thêm chi tiết tại entry "Lộc Tồn + Bác Sĩ thủ
Mệnh" và "Kình Đà giáp Lộc Tồn".`,

    chiTiet: `## Học hành - đỗ đạt

Khi có **Hóa Khoa / Thái Âm / Thái Dương / Văn Xương / Văn Khúc**:
- **Học hành đỗ đạt**, dễ thành danh
- Lĩnh vực đòi hỏi **chuyên môn cao**: nghiên cứu, giáo dục
- **Công danh vang lừng rạng rỡ**

## Năng khiếu Y học

Khi có **Phúc thiện tinh** (Thiên Lương / Thiên Tướng / Quang Quý / Quan Phúc):
- Có **năng khiếu y học, giáo dục, nghệ thuật**
- Được trời phú đôi tay **"chữa lành"**
- Thuận lợi theo đuổi **y dược**: trở thành **thầy thuốc, bác sĩ giỏi**, đóng
  góp giá trị cao cho cộng đồng

## Sức khỏe

Rất biết **chăm sóc bản thân**, duy trì **lối sống khoa học, điều độ**.

Nếu chẳng may **lâm bệnh**: dễ **gặp được thầy giỏi** hoặc tìm ra **phương
pháp điều trị hiệu quả**: mau bình phục.`,
  },

  // ============================================================
  // 3. Bác Sĩ tại Phụ Mẫu
  // ============================================================
  {
    id: 'bac-si-tai-phu-mau',
    title: 'Bác Sĩ tại Phụ Mẫu',
    sao: ['Bác Sĩ'],
    cung: ['Phụ Mẫu'],
    doUuTien: 75,
    tomTat: `**Bác Sĩ cư Phụ Mẫu:** cha mẹ là người **có học thức, điềm đạm,
có đạo đức**, biết **vun vén** và dạy con cái theo nề nếp.

Gia đình **khá giả** — cha mẹ tích được nhiều tài sản và để lại cho **con
cháu sau này**.

Đương số được sinh trong môi trường **đủ đầy**, được chăm sóc và định hướng
rõ ràng: **không phải chật vật mưu sinh**.

Bản thân đôi khi **dễ ốm đau, mắc bệnh vặt khi còn nhỏ** (đặc trưng cấu hình
sao quanh Bác Sĩ - Lộc Tồn - Đà La - Kình Dương).`,
  },
  {
    id: 'bac-si-tai-phu-mau-luc-cat',
    title: 'Bác Sĩ tại Phụ Mẫu + Lục Cát - Cha mẹ thăng tiến',
    sao: ['Bác Sĩ'],
    cung: ['Phụ Mẫu'],
    ketHop: ['Tả Phù', 'Hữu Bật', 'Văn Xương', 'Văn Khúc', 'Thiên Khôi', 'Thiên Việt'],
    doUuTien: 70,
    tomTat: `**Bác Sĩ + Lục Cát (Tả Phù / Hữu Bật / Văn Xương / Văn Khúc /
Thiên Khôi / Thiên Việt) tại Phụ Mẫu:** cha mẹ càng lớn tuổi càng **thăng
tiến**, làm **chủ doanh nghiệp, lãnh đạo, quan chức cao cấp**.`,
  },

  // ============================================================
  // 4. Bác Sĩ tại Phúc Đức
  // ============================================================
  {
    id: 'bac-si-tai-phuc-duc',
    title: 'Bác Sĩ tại Phúc Đức',
    sao: ['Bác Sĩ'],
    cung: ['Phúc Đức'],
    doUuTien: 78,
    tomTat: `**Bác Sĩ cư Phúc Đức:** dòng họ có **học thức cao, thành đạt**,
phúc phần **lớn**.

Trong họ dễ có **phần mộ hoặc đất hương hỏa LINH THIÊNG**: mang lại **sung
túc, phú quý** cho con cháu. **Mồ mả KẾT PHÁT**.

Đương số được hưởng **may mắn từ phúc đức tổ tiên**, có thể **thừa hưởng
đất đai/tài sản** từ ông bà.

Đương số có **duyên với người có địa vị, quyền cao chức trọng** trong xã
hội, gặp nhiều **may mắn** trong cuộc sống.

Người **hôn phối giỏi giang, có chức quyền, cầm con dấu**, gia đình bên hôn
phối đều là **người thành đạt, tài chính mạnh**.`,
  },

  // ============================================================
  // 5. Bác Sĩ tại Điền Trạch
  // ============================================================
  {
    id: 'bac-si-tai-dien-trach',
    title: 'Bác Sĩ tại Điền Trạch',
    sao: ['Bác Sĩ'],
    cung: ['Điền Trạch'],
    doUuTien: 75,
    tomTat: `**Bác Sĩ cư Điền Trạch:** có **duyên có lộc với đất cát**, được
**thừa hưởng điền sản** từ tổ tiên để lại.

Quan tâm chăm lo cho ngôi nhà: nhà cửa **gọn gàng, sạch sẽ, thanh nhã**,
trong nhà có **THƯ PHÒNG** (đặc trưng Bác Sĩ).`,
  },
  {
    id: 'bac-si-tai-dien-trach-co-dong-xuong-khuc-dao',
    title: 'Bác Sĩ tại Điền Trạch + Cơ/Đồng/Xương Khúc/Đào - Kiến trúc sư',
    sao: ['Bác Sĩ'],
    cung: ['Điền Trạch'],
    ketHop: ['Thiên Cơ', 'Thiên Đồng', 'Văn Xương', 'Văn Khúc', 'Đào Hoa'],
    doUuTien: 70,
    tomTat: `**Bác Sĩ + Thiên Cơ / Thiên Đồng / Văn Xương / Văn Khúc / Đào Hoa
tại Điền Trạch:**
- Có **tài năng nghệ thuật** liên quan **kiến trúc, thiết kế**
- Trở thành **kiến trúc sư, kỹ sư, nghệ sĩ nổi tiếng**
- Hoặc bén duyên với **bất động sản** lâu dài`,
  },

  // ============================================================
  // 6. Bác Sĩ tại Quan Lộc
  // ============================================================
  {
    id: 'bac-si-tai-quan-loc',
    title: 'Bác Sĩ tại Quan Lộc',
    sao: ['Bác Sĩ'],
    cung: ['Quan Lộc'],
    doUuTien: 78,
    tomTat: `**Bác Sĩ cư Quan Lộc:** **may mắn** trong công việc và công danh.

Phong cách làm việc:
- **Cẩn trọng, tỉ mỉ**, tác phong **điềm đạm**
- Có **trí tuệ, tài năng**
- Được **cấp trên tin tưởng** và giao **trọng trách quan trọng**

Thường được đảm nhận **chức vụ cao**, hoặc **làm chủ doanh nghiệp**.

**Lòng từ bi nhân hậu**, yêu thương giúp đỡ mọi người.`,
  },
  {
    id: 'bac-si-tai-quan-loc-phuc-thien',
    title: 'Bác Sĩ tại Quan Lộc + Phúc thiện tinh - Bác sĩ thầy thuốc',
    sao: ['Bác Sĩ'],
    cung: ['Quan Lộc'],
    ketHop: ['Thiên Lương', 'Thiên Tướng', 'Ân Quang', 'Thiên Quý', 'Thiên Quan', 'Thiên Phúc'],
    doUuTien: 73,
    tomTat: `**Bác Sĩ + Phúc thiện tinh (Thiên Lương / Thiên Tướng / Ân Quang /
Thiên Quý / Thiên Quan / Thiên Phúc) tại Quan Lộc:** làm nghề **bác sĩ, thầy
thuốc, giáo viên** — được nhiều người **kính trọng**.`,
  },

  // ============================================================
  // 7. Bác Sĩ tại Nô Bộc
  // ============================================================
  {
    id: 'bac-si-tai-no-boc',
    title: 'Bác Sĩ tại Nô Bộc',
    sao: ['Bác Sĩ'],
    cung: ['Nô Bộc'],
    doUuTien: 75,
    tomTat: `**Bác Sĩ cư Nô:** nhận được **tín nhiệm và kính trọng** từ khách
hàng, đồng nghiệp, cấp trên.

Hay được bạn bè, đồng nghiệp, cấp dưới **yêu quý, giúp đỡ, tặng quà**, được
**mời nhậu nhẹt ăn uống**.

Có **lộc trong làm ăn, chung vốn** với bạn bè.

Khả năng **làm ăn chung** với bạn bè, đồng nghiệp được phát huy mạnh.

Bạn bè đồng nghiệp **có học thức, chuyên môn giỏi**.`,
  },
  {
    id: 'bac-si-tai-no-boc-khoi-viet',
    title: 'Bác Sĩ tại Nô Bộc + Khôi Việt - Bạn quyền cao',
    sao: ['Bác Sĩ'],
    cung: ['Nô Bộc'],
    ketHop: ['Thiên Khôi', 'Thiên Việt', 'Ân Quang', 'Thiên Quý'],
    doUuTien: 70,
    tomTat: `**Bác Sĩ + Quý tinh (Thiên Khôi / Thiên Việt / Ân Quang / Thiên
Quý) tại Nô Bộc:** bạn bè là **người có địa vị, quyền cao chức trọng** trong
xã hội — có tiếng nói trong xã hội.`,
  },

  // ============================================================
  // 8. Bác Sĩ tại Thiên Di
  // ============================================================
  {
    id: 'bac-si-tai-thien-di',
    title: 'Bác Sĩ tại Thiên Di',
    sao: ['Bác Sĩ'],
    cung: ['Thiên Di'],
    doUuTien: 75,
    tomTat: `**Bác Sĩ cư Thiên Di:** có **tài năng và khả năng ngoại giao xuất
sắc**.

Đặc điểm:
- **Khôn ngoan, cẩn thận, khiêm nhường**
- Dễ **thích nghi với môi trường mới**
- Hay **gặp may mắn**

Hay được **mời đi ăn, rủ đi chơi xa**, tham dự **tiệc, sự kiện**, gặp gỡ
các **mối quan hệ có giá trị**.

Công việc nhờ vậy cũng **thuận buồm xuôi gió**, mở ra nhiều cơ hội phát
triển lâu dài.`,
  },

  // ============================================================
  // 9. Bác Sĩ tại Tật Ách
  // ============================================================
  {
    id: 'bac-si-tai-tat-ach',
    title: 'Bác Sĩ tại Tật Ách - Sức khỏe được bảo vệ',
    sao: ['Bác Sĩ'],
    cung: ['Tật Ách'],
    doUuTien: 78,
    tomTat: `**Bác Sĩ cư Tật Ách:** dấu hiệu TỐT — **sức khỏe được bảo vệ**.

Đương số:
- **Ý thức cao** trong việc chăm sóc bản thân
- Duy trì thói quen **sống khoa học**
- Khi có bệnh thường **may tìm được thầy thuốc giỏi**, **mau khỏi bệnh**

**Đặc trưng:** **mất tiền để giải trừ bệnh tật** (giống Lộc Tồn cư Tật Ách).

Trong hoàn cảnh **tai họa, nạn dịch**: luôn biết cách **kiếm tiền xung
quanh hoàn cảnh đó** (ví dụ: bán khẩu trang đúng mùa covid).

Một số người còn có cơ hội **kiếm thêm thu nhập từ tình huống đặc thù** như
dịch bệnh.`,
  },

  // ============================================================
  // 10. Bác Sĩ tại Tài Bạch
  // ============================================================
  {
    id: 'bac-si-tai-tai-bach',
    title: 'Bác Sĩ tại Tài Bạch',
    sao: ['Bác Sĩ'],
    cung: ['Tài Bạch'],
    doUuTien: 78,
    tomTat: `**Bác Sĩ cư Tài Bạch:** có **kiến thức sâu rộng** về tài chính,
tiền bạc — **cẩn thận, biết tính toán**, hiếm khi tiêu xài hoang phí.`,
  },
  {
    id: 'bac-si-tai-tai-bach-tai-tinh',
    title: 'Bác Sĩ tại Tài Bạch + Tài tinh - Ngành tài chính kế toán',
    sao: ['Bác Sĩ'],
    cung: ['Tài Bạch'],
    ketHop: ['Vũ Khúc', 'Thiên Phủ', 'Lộc Tồn', 'Hóa Lộc'],
    doUuTien: 73,
    tomTat: `**Bác Sĩ + Tài tinh (Vũ Khúc / Thiên Phủ / Lộc Tồn / Hóa Lộc) tại
Tài Bạch:** công việc thường liên quan **ngành tài chính, kế toán**. Kiếm tiền
**giỏi**, **quản lý tài chính tốt**.`,
  },
  {
    id: 'bac-si-tai-tai-bach-luc-cat',
    title: 'Bác Sĩ tại Tài Bạch + Lục Cát - Dòng tiền ổn định',
    sao: ['Bác Sĩ'],
    cung: ['Tài Bạch'],
    ketHop: ['Tả Phù', 'Hữu Bật', 'Văn Xương', 'Văn Khúc', 'Thiên Khôi', 'Thiên Việt'],
    doUuTien: 73,
    tomTat: `**Bác Sĩ + Lục Cát (Tả Phù / Hữu Bật / Văn Xương / Văn Khúc /
Thiên Khôi / Thiên Việt) tại Tài Bạch:** dòng tiền **tăng trưởng đều đặn, ổn
định**, ít gặp sóng gió bấp bênh.`,
  },
  {
    id: 'bac-si-tai-tai-bach-vu-khuc-co-qua-dau-quan',
    title: 'Bác Sĩ tại Tài Bạch + Vũ Khúc/Cô Quả/Đẩu Quân - Keo kiệt',
    sao: ['Bác Sĩ'],
    cung: ['Tài Bạch'],
    ketHop: ['Vũ Khúc', 'Cô Thần', 'Quả Tú', 'Đẩu Quân'],
    doUuTien: 73,
    tomTat: `**Bác Sĩ + Vũ Khúc / Cô Thần / Quả Tú / Đẩu Quân tại Tài Bạch:**
trở thành người **KEO KIỆT, TÍNH TOÁN CHI LI**.`,
  },

  // ============================================================
  // 11. Bác Sĩ tại Tử Tức
  // ============================================================
  {
    id: 'bac-si-tai-tu-tuc',
    title: 'Bác Sĩ tại Tử Tức',
    sao: ['Bác Sĩ'],
    cung: ['Tử Tức'],
    doUuTien: 75,
    tomTat: `**Bác Sĩ cư Tử Tức:** đường con cái có phần **chậm hoặc muộn** —
**hiếm muộn**, đôi khi phải nhờ **can thiệp của y học** để hỗ trợ mang thai.

Bù lại con cái sinh ra **thông minh, ngoan ngoãn, hiếu thảo, thành đạt**.`,
  },
  {
    id: 'bac-si-tai-tu-tuc-xuong-khuc',
    title: 'Bác Sĩ tại Tử Tức + Văn Xương/Văn Khúc - Con giỏi lãnh đạo',
    sao: ['Bác Sĩ'],
    cung: ['Tử Tức'],
    ketHop: ['Văn Xương', 'Văn Khúc'],
    doUuTien: 70,
    tomTat: `**Bác Sĩ + Văn Xương / Văn Khúc tại Tử Tức:**
- Học hành **giỏi giang**
- Có **tố chất lãnh đạo**
- Mang lại **niềm tự hào lớn** cho gia đình`,
  },

  // ============================================================
  // 12. Bác Sĩ tại Phu Thê
  // ============================================================
  {
    id: 'bac-si-tai-phu-the',
    title: 'Bác Sĩ tại Phu Thê',
    sao: ['Bác Sĩ'],
    cung: ['Phu Thê'],
    doUuTien: 78,
    tomTat: `**Bác Sĩ cư Phu Thê:** **kén chọn** trong việc chọn vợ chọn chồng,
**tiêu chuẩn cao**.

Đặc trưng: **kết hôn MUỘN**, người hôn phối dễ **HƠN tuổi mình**.

Người hôn phối:
- **Điềm đạm, giỏi giang**
- Có **học thức cao**
- Có **tiền tài, địa vị, chức tước** trong xã hội
- Thậm chí **nhiều tài sản hoặc quyền cao chức trọng**

Hôn nhân nhìn chung **ổn định, bền chặt** — xây dựng dựa trên **tôn trọng
và lý trí**, ít bị chi phối bởi **cảm xúc bồng bột**.`,
  },

  // ============================================================
  // 13. Bác Sĩ tại Huynh Đệ
  // ============================================================
  {
    id: 'bac-si-tai-huynh-de',
    title: 'Bác Sĩ tại Huynh Đệ',
    sao: ['Bác Sĩ'],
    cung: ['Huynh Đệ'],
    doUuTien: 75,
    tomTat: `**Bác Sĩ cư Huynh Đệ:** **ít anh chị em**, **chênh lệch tuổi nhiều**
so với anh chị em.

Anh chị em là người **có học thức, thành đạt**, sự nghiệp **vững vàng** —
hay **giúp đỡ mình**.

Có thể trở thành **chỗ dựa tốt**, hậu thuẫn cả **vật chất lẫn tinh thần**
trong những giai đoạn quan trọng.`,
  },

  // ============================================================
  // 14. Bác Sĩ + Hóa Khoa
  // ============================================================
  {
    id: 'bac-si-hoa-khoa',
    title: 'Bác Sĩ + Hóa Khoa - Học hành chuyên môn cao',
    sao: ['Bác Sĩ'],
    ketHop: ['Hóa Khoa'],
    doUuTien: 60,
    tomTat: `**Bác Sĩ + Hóa Khoa:** người có **học hành, chuyên môn CAO**.

- Hai sao có đặc tính giống nhau (chủ học hành, giải bệnh) khi gặp nhau =
**cộng hưởng** mạnh:
- **Đỗ đạt khoa cử**
- Chuyên môn **được công nhận chính thức**
- Bằng cấp cao, học vị tốt

Đặc biệt thuận lợi cho **nghiên cứu, giáo dục, y dược**.`,
  },

  // ============================================================
  // 15. Bác Sĩ + Âm Dương + Xương Khúc - Thành danh nổi tiếng
  // ============================================================
  {
    id: 'bac-si-am-duong-xuong-khuc',
    title: 'Bác Sĩ + Âm Dương + Xương Khúc - Thành danh nổi tiếng',
    sao: ['Bác Sĩ'],
    ketHop: ['Thái Âm', 'Thái Dương', 'Văn Xương', 'Văn Khúc'],
    doUuTien: 60,
    tomTat: `**Bác Sĩ + Thái Âm + Thái Dương + Văn Xương + Văn Khúc:**

- **THÀNH DANH, NỔI TIẾNG**
- **Công danh vang lừng rạng rỡ**

- Cách cục đẹp của người làm trong lĩnh vực **trí thức, học giả, nghiên cứu,
giáo dục** — danh tiếng được công nhận rộng rãi.`,
  },

  // ============================================================
  // 16. Bác Sĩ + phúc thiện tinh - năng khiếu y học
  // ============================================================
  {
    id: 'bac-si-phuc-thien-tinh',
    title: 'Bác Sĩ + Phúc Thiện tinh - Năng khiếu y học, làm thầy thuốc',
    sao: ['Bác Sĩ'],
    ketHop: ['Thiên Lương', 'Thiên Tướng', 'Ân Quang', 'Thiên Quý', 'Thiên Quan', 'Thiên Phúc'],
    doUuTien: 62,
    tomTat: `**Bác Sĩ + Thiên Lương + Thiên Tướng + Quang Quý + Quan Phúc:**
năng khiếu trong các lĩnh vực **Y HỌC, GIÁO DỤC, NGHỆ THUẬT**.

- Đương số dường như được trời phú đôi tay **"CHỮA LÀNH"**:
- Thuận lợi theo đuổi ngành **Y, Dược**
- Dễ trở thành **thầy thuốc, bác sĩ GIỎI**
- Đóng góp **giá trị cao cho cộng đồng**

- Đúng nghĩa cái tên "Bác Sĩ" — sao của người **làm nghề chữa bệnh, dạy dỗ,
cứu giúp**.`,
  },

  // ============================================================
  // 17. Bác Sĩ vào hạn
  // ============================================================
  {
    id: 'bac-si-vao-han',
    title: 'Bác Sĩ vào hạn',
    sao: ['Bác Sĩ'],
    doUuTien: 50,
    tomTat: `**Hạn có Bác Sĩ:** giai đoạn **trau dồi kiến thức**, **xây dựng
nền tảng** và **tích lũy tài chính**.

**Bác Sĩ + Lộc Tồn** (đồng cung) ám chỉ giai đoạn **NO ẤM và THỊNH VƯỢNG**
về vật chất.

**NGUYÊN TẮC:** cung vận có Bác Sĩ + Lộc Tồn **luôn ở thế bị KẸP giữa
KÌNH DƯƠNG và ĐÀ LA** (do quy luật an sao):
- Thời gian **đầu** có thể gặp một số **thử thách hoặc áp lực**
- Nếu **kiên trì vượt qua**: gặt hái **thành quả bền vững**
- Làm **bàn đạp vững chắc** cho tương lai

Hạn có Bác Sĩ + bệnh tật: mau bình phục, gặp được thầy giỏi.`,
  },
];
